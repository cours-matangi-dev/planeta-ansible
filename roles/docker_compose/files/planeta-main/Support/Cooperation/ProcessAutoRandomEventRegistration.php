 ...
 * @version 3.6
 *     - 2019-06-17 : v3.3.
 *     - 2020-12-24 : v3.6. Taken into account CONF_COOP_EVENT_RANDOM_AUTO_FAMILIES_NB_MAX_REGISTRATIONS to allow
 *                    to register several times per school year a same family to a given event type ID
 *
 * @since 2019-06-17
 */
 ...
 if (isSet($_SESSION["SupportMemberID"]))
 {
     ...
     if ((md5($EventID) == $CryptedID) && (isExistingEvent($DbCon, $EventID)))
     {
         ...
         // Get the max number of registrations of eahc family per school year for the given event type ID
         $iMaxRegistrationsPerSchoolYear = 1;
         if ((isset($CONF_COOP_EVENT_RANDOM_AUTO_FAMILIES_NB_MAX_REGISTRATIONS[$RecordEvent['EventTypeID']]))
             && ($CONF_COOP_EVENT_RANDOM_AUTO_FAMILIES_NB_MAX_REGISTRATIONS[$RecordEvent['EventTypeID']] > 0))
         {
             // Another max value is set for this event type ID
             $iMaxRegistrationsPerSchoolYear = $CONF_COOP_EVENT_RANDOM_AUTO_FAMILIES_NB_MAX_REGISTRATIONS[$RecordEvent['EventTypeID']];
         }
         ...
         $ArrayRegisteredFamilies = array_fill(1, $iMaxRegistrationsPerSchoolYear, array());
         ...
         if ((isset($ArraySameTypesEvents['EventID'])) && (!empty($ArraySameTypesEvents['EventID'])))
         {
             ...
             if ((isset($ArrayEventRegistrations['EventRegistrationID'])) && (!empty($ArrayEventRegistrations['EventRegistrationID'])))
             {
                 ...
                 {
                     // We count how many times each family has been registered on this event type ID
                     for($t = 1; $t <= $iMaxRegistrationsPerSchoolYear; $t++)
                     {
                         if (!in_array($ArrayEventRegistrations['FamilyID'][$er], $ArrayRegisteredFamilies[$t]))
                         {
                             $ArrayRegisteredFamilies[$t][] = $ArrayEventRegistrations['FamilyID'][$er];
                             break;
                         }
                     }
         ...
         {
             // We select families with less registrations if there are enough
             $iNbFamiliesToSelect = max(0, $RecordEvent['EventMaxParticipants'] - $iNbRegisteredFamiliesOnCurrentEvent);
             for($t = 1; $t <= $iMaxRegistrationsPerSchoolYear; $t++)
             {
                 $ArrayDiffFamilies = array_diff($ArrayActivatedFamilies['FamilyID'], $ArrayRegisteredFamilies[$t]);

                 // Random shuffle families
                 shuffle($ArrayDiffFamilies);

                 // Add families to already selected families
                 $ArraySelectedFamilies = array_merge($ArraySelectedFamilies, $ArrayDiffFamilies);

                 if (count($ArraySelectedFamilies) >= $iNbFamiliesToSelect)
                 {
                     // We stop the search
                     break;
                 }
             ...
             $ArraySelectedFamilies = array_slice($ArraySelectedFamilies, 0, $iNbFamiliesToSelect);
         }
