 ...
 *     - 2021-05-18 : v3.6. Taken into account FamilyMainPicture and FamilySecondPicture fields and patch a bug about
 *                    the annual balance when a family updates the data
 *     - 2022-06-06 : v3.8. Taken into account FamilyMonthlyNurseryMode field
 *
 * @since 2012-01-23
 */
 ...
 if (!empty($_POST["bSubmit"]))
 {
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         if (isExistingFamily($DbCon, $Id))
         {
             ...
             // Get the previous values of the family (ex : nb of members with or without power)
         ...
         else
         {
             ...
             $RecordOldFamily = array(
                                      'FamilyNbMembers' => 1,
                                      'FamilyNbPoweredMembers' => 2
                                     );
         }
         ...
         // Get the monthly nursery mode
         $MonthlyNurseryMode = existedPOSTFieldValue("lMonthlyNurseryMode", NULL);
         if (!is_null($MonthlyNurseryMode))
         {
             $MonthlyNurseryMode = strip_tags($MonthlyNurseryMode);
         }

         $NbNumbers = existedPOSTFieldValue("sNbMembers", NULL);
         if (is_Null($NbNumbers))
         {
             // The form field isn't there : we get the current value in DB for the family
             // We get the current value in DB for the family
             $NbNumbers = $RecordOldFamily['FamilyNbMembers'];
         }
         else
         {
         ...
         {
             // The form field isn't there : we get the current value in DB for the family
             $NbPoweredNumbers = $RecordOldFamily['FamilyNbPoweredMembers'];
         }
         else
         {
         ...
         // We taken into account the uploaded files
         $MainPicture = NULL;
         $bMainPictureError = FALSE;
         if ((isset($_FILES)) && (isset($_FILES["fMainPicture"])) && ($_FILES["fMainPicture"]["name"] != ""))
         {
             // We give a valide name to the uploaded file
             $_FILES["fMainPicture"]["name"] = formatFilename($_FILES["fMainPicture"]["name"]);

             // Check if the file owns an allowed extension
             if (isFileOwnsAllowedExtension($_FILES["fMainPicture"]["name"], array('jpg', 'jpeg', 'png')))
             {
                 if (is_uploaded_file($_FILES["fMainPicture"]["tmp_name"]))
                 {
                     // The file name is converted to md5 and we add a prefix
                     $MainPicture = $_FILES["fMainPicture"]["name"];
                     $iExtPos = strrpos($MainPicture, '.');
                     $MainPicture = md5(substr($MainPicture, 0, $iExtPos)).substr($MainPicture, $iExtPos);
                     $MainPicture = "F$FamilyID-1_".$MainPicture;

                     if ($_FILES["fMainPicture"]["size"] > $CONF_UPLOAD_UPLOADED_FILES_MAXSIZE)
                     {
                         // Error : file to big
                         $ContinueProcess = FALSE;
                         $bMainPictureError = TRUE;
                     }
                 }
             }
             else
         ...
         $SecondPicture = NULL;
         $bSecondPictureError = FALSE;
         if ((isset($_FILES)) && (isset($_FILES["fSecondPicture"])) && ($_FILES["fSecondPicture"]["name"] != ""))
         {
             // We give a valide name to the uploaded file
             $_FILES["fSecondPicture"]["name"] = formatFilename($_FILES["fSecondPicture"]["name"]);

             // Check if the file owns an allowed extension
             if (isFileOwnsAllowedExtension($_FILES["fSecondPicture"]["name"], array('jpg', 'jpeg', 'png')))
             {
                 if (is_uploaded_file($_FILES["fSecondPicture"]["tmp_name"]))
                 {
                     // The file name is converted to md5 and we add a prefix
                     $SecondPicture = $_FILES["fSecondPicture"]["name"];
                     $iExtPos = strrpos($SecondPicture, '.');
                     $SecondPicture = md5(substr($SecondPicture, 0, $iExtPos)).substr($SecondPicture, $iExtPos);
                     $SecondPicture = "F$FamilyID-2_".$SecondPicture;

                     if ($_FILES["fSecondPicture"]["size"] > $CONF_UPLOAD_UPLOADED_FILES_MAXSIZE)
                     {
                         // Error : file to big
                         $ContinueProcess = FALSE;
                         $bSecondPictureError = TRUE;
                     }
                 }
             }
             else
         ...
         if ($ContinueProcess)
         {
             ...
             {
                 // We move the main picture file in the right directory
                 @move_uploaded_file($_FILES["fMainPicture"]["tmp_name"],
                                     $CONF_UPLOAD_FAMILY_PICTURE_FILES_DIRECTORY_HDD.$MainPicture);
             }

             if (!empty($SecondPicture))
             {
                 // We move the main picture file in the right directory
                 @move_uploaded_file($_FILES["fSecondPicture"]["tmp_name"],
                                     $CONF_UPLOAD_FAMILY_PICTURE_FILES_DIRECTORY_HDD.$SecondPicture);
             }
             ...
             $FamilyID = dbUpdateFamily($DbCon, $FamilyID, NULL, $Lastname, $TownID, $MainEmail, $SecondEmail, $NbNumbers,
                                        ...
                                        $MonthyContributionMode, $MonthlyNurseryMode, $MainEmailContactAllowed, $SecondEmailContactAllowed,
                                        $MainEmailInCommittee, $SecondEmailInCommittee, NULL, $MainPicture, $SecondPicture);
