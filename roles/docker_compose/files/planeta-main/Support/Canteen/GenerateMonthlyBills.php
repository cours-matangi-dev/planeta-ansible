 ...
 *     - 2022-06-07 : v3.8. Taken into account new formats of CONF_NURSERY_PRICES and CONF_CANTEEN_PRICES variables and
 *                    taken into account FamilyMonthlyNurseryMode field
 *
 * @since 2012-02-21
 */
 ...
 if ((!empty($_POST["bGenerate"])) || (!empty($_POST["bMissingBills"])))
 {
     ...
     // We detect the canteen prices mode for the current school year
     // M  --> Monthly prices
     // NB --> each canteen registration must be paid
     // UP --> each canteen registration has a unique price
     // GR --> price is in relation with the child grade
     // CB --> price is in relation with the contribution mode of the family
     $ArrayTmpCanteenModes = array();
     $_CANTEEN_PRICES_MODES_ = array('M_NB_UP');
     $_CANTEEN_PRICES_TO_USE_ = array();
     if ((isset($CONF_CANTEEN_PRICES[$SchoolYear])) && (!empty($CONF_CANTEEN_PRICES[$SchoolYear])))
     {
         $ArrayTmpCanteenModes = array_keys($CONF_CANTEEN_PRICES[$SchoolYear]);
         if (isInteger($ArrayTmpCanteenModes[0]))
         {
             // No label for the first canteen mode, so it's the default mode to pay canteen registrations
             // and there is only one defined mode to pay
             $_CANTEEN_PRICES_MODES_ = array('M_NB_UP');
             $_CANTEEN_PRICES_TO_USE_[0] = $CONF_CANTEEN_PRICES[$SchoolYear];
         }
         else
         {
             // There are several modes to pay canteen registrations
             foreach($ArrayTmpCanteenModes as $tcm => $CurrentCanteenMode)
             {
                 // Monthly price mode
                 if ($tcm == 0)
                 {
                     $_CANTEEN_PRICES_MODES_ = array('M_NB');

                     switch(substr($ArrayTmpCanteenModes[0], 0, 2))
                     {
                         case 'G_':
                             // Canteen prices in relation with child grades...
                             $_CANTEEN_PRICES_MODES_[0] .= '_GR';
                             $ArrayKeysSecondCanteenParams = array_keys($CONF_CANTEEN_PRICES[$SchoolYear][$CurrentCanteenMode]);

                             switch(substr($ArrayKeysSecondCanteenParams[0], 0, 2))
                             {
                                 case 'C_':
                                     // ... and canteen prices in relation with the contribution mode
                                     $_CANTEEN_PRICES_MODES_[0] .= '_CB';
                                     break;

                                 default:
                                     $_CANTEEN_PRICES_MODES_[0] .= '_UP';
                                     break;
                             }
                 ...
                 $_CANTEEN_PRICES_TO_USE_[0][$CurrentCanteenMode] = $CONF_CANTEEN_PRICES[$SchoolYear][$CurrentCanteenMode];
             }

             unset($ArrayTmp, $ArrayKeysFirstCanteenParams);
         }
     }
     ...
     // We detect the nursery prices mode for the current school year
     // M  --> Monthly prices
     // A  --> Annual prices
     // NB --> each nursery registration must be paid
     // UP --> each nuresery timeslot has a unique price
     // PK --> nursery registrations must be paid by a unique price (package)
     // GR --> price or package is in relation with the child grade
     // CB --> price or package is in relation with the contribution mode of the family
     $ArrayTmpNurseryModes = array();
     $_NURSERY_PRICES_MODES_ = array('M_NB_UP');
     $_NURSERY_PRICES_TO_USE_ = array();
     if ((isset($CONF_NURSERY_PRICES[$SchoolYear])) && (!empty($CONF_NURSERY_PRICES[$SchoolYear])))
     {
         $ArrayTmpNurseryModes = array_keys($CONF_NURSERY_PRICES[$SchoolYear]);
         if (isInteger($ArrayTmpNurseryModes[0]))
         {
             // No label for the first nuresery mode, so it's the default mode to pay nursery registrations
             // and there is only one defined mode to pay
             $_NURSERY_PRICES_MODES_ = array('M_NB_UP');
             $_NURSERY_PRICES_TO_USE_[0] = $CONF_NURSERY_PRICES[$SchoolYear];
         }
         else
         {
             // There are labels, so there are several modes to pay nursery registrations
             foreach($ArrayTmpNurseryModes as $tnm => $CurrentNurseryMode)
             {
                 // Monthly or annual prices mode ?
                 $ArrayTmp = explode('_', $CurrentNurseryMode);
                 $sTypeMode = $ArrayTmp[1];

                 if ($tnm == 0)
                 {
                     // The first mode is always a 'NB' mode
                     $_NURSERY_PRICES_MODES_ = array($sTypeMode.'_NB');
                 }
                 else
                 {
                     $_NURSERY_PRICES_MODES_[$tnm] = $sTypeMode.'_PK';
                 }

                 $_NURSERY_PRICES_TO_USE_[$tnm] = $CONF_NURSERY_PRICES[$SchoolYear][$CurrentNurseryMode];

                 $ArrayKeysFirstNurseryParams = array_keys($CONF_NURSERY_PRICES[$SchoolYear][$CurrentNurseryMode]);
                 switch(substr($ArrayKeysFirstNurseryParams[0], 0, 2))
                 {
                     case 'G_':
                         // Nursery prices in relation with child grades...
                         $_NURSERY_PRICES_MODES_[$tnm] .= '_GR';
                         $ArrayKeysSecondNurseryParams = array_keys($CONF_NURSERY_PRICES[$SchoolYear][$CurrentNurseryMode][$ArrayKeysFirstNurseryParams[0]]);

                         switch(substr($ArrayKeysSecondNurseryParams[0], 0, 2))
                         {
                             case 'C_':
                                 // ... and nursery prices in relation with the contribution mode
                                 $_NURSERY_PRICES_MODES_[$tnm] .= '_CB';
                                 break;

                             default:
                                 $_NURSERY_PRICES_MODES_[$tnm] .= '_UP';
                                 break;
                         }

                         unset($ArrayKeysSecondNurseryParams);
                         break;
                 }
             }

             unset($ArrayTmp, $ArrayKeysFirstNurseryParams);
         }
     }
     ...
     if ((isset($ArrayFamilies['FamilyID'])) && (!empty($ArrayFamilies['FamilyID'])))
     {
         ...
         foreach($ArrayFamilies['FamilyID'] as $f => $FamilyID)
         {
             // We get the canteen mode of the family
             $FamilyMonthlyCanteenMode = 0;

             // We get the nursery mode of the family
             $FamilyMonthlyNurseryMode = $ArrayFamilies['FamilyMonthlyNurseryMode'][$f];
             ...
             $_CANTEEN_FAMILY_PRICE_MODE_ = $_CANTEEN_PRICES_MODES_[$FamilyMonthlyCanteenMode];
             $_CANTEEN_FAMILY_PRICES_TO_USE_ = $_CANTEEN_PRICES_TO_USE_[$FamilyMonthlyCanteenMode];
             ...
             {
                 switch($_CANTEEN_FAMILY_PRICE_MODE_)
                 {
                     case 'M_NB_UP';
                         // Monthly prices. Each canteen registration must be paid with a unique price
                         foreach($ArrayCanteenRegistrations['CanteenRegistrationID'] as $cr => $CanteenRegistrationID)
                         {
                             ...
                             if ($ArrayCanteenRegistrations['CanteenRegistrationValided'][$cr] == 0)
                             {
                                 ...
                                 $BillWithoutMealAmount += $_CANTEEN_FAMILY_PRICES_TO_USE_[0];
                             }
                             else
                             {
                                              ...
                                              $CONF_CANTEEN_PRICES_CONCERNED_MEAL_TYPES))
                                 {
                                     ...
                                     $BillCanteenAmount += $_CANTEEN_FAMILY_PRICES_TO_USE_[0] + $_CANTEEN_FAMILY_PRICES_TO_USE_[1];
                                 }
                                 else
                                 {
                                     // Only price of the nursery for the lunch
                                     $BillWithoutMealAmount += $_CANTEEN_FAMILY_PRICES_TO_USE_[1];
                                 }
                             }
                     ...
                     case 'M_NB_GR_UP':
                         // Monthly prices. Each canteen registration must be paid with a unique price in relation with child grade
                         foreach($ArrayCanteenRegistrations['CanteenRegistrationID'] as $cr => $CanteenRegistrationID)
                         {
                             $GradeToUse = 'G_'.$ArrayCanteenRegistrations['ChildGrade'][$cr];

                             // Check if the canteen registration is valided
                             if ($ArrayCanteenRegistrations['CanteenRegistrationValided'][$cr] == 0)
                             {
                                 // Not valided : only price of the lunch
                                 $BillWithoutMealAmount += $_CANTEEN_FAMILY_PRICES_TO_USE_[$GradeToUse][0];
                             }
                             else
                             {
                                 // Valided : the type of meal is concerned by the price of the lunch ?
                                 ...
                                 {
                                     // Price of the lunch + price of the nursery for the lunch
                                     $BillCanteenAmount += $_CANTEEN_FAMILY_PRICES_TO_USE_[$GradeToUse][0] + $_CANTEEN_FAMILY_PRICES_TO_USE_[$GradeToUse][1];
                                 }
                                 else
                                 {
                                     // Only price of the nursery for the lunch
                                     $BillWithoutMealAmount += $_CANTEEN_FAMILY_PRICES_TO_USE_[$GradeToUse][1];
                                 }
                             }
                     ...
                     case 'M_NB_GR_CB':
                         // Monthly prices. Each canteen registration must be paid with a unique price in relation with child grade
                         // and monthy contribution mode of the family
                         $ContribToUse = 'C_'.$ArrayFamilies['FamilyMonthlyContributionMode'][$f];
                         ...
                         foreach($ArrayCanteenRegistrations['CanteenRegistrationID'] as $cr => $CanteenRegistrationID)
                         {
                             ...
                             {
                                 // Not valided : only price of the lunch
                                 $BillWithoutMealAmount += $_CANTEEN_FAMILY_PRICES_TO_USE_[$GradeToUse][$ContribToUse][0];
                             }
                             else
                             {
                                 // Valided : the type of meal is concerned by the price of the lunch ?
                                 ...
                                 {
                                     // Price of the lunch + price of the nursery for the lunch
                                     $BillCanteenAmount += $_CANTEEN_FAMILY_PRICES_TO_USE_[$GradeToUse][$ContribToUse][0]
                                                           + $_CANTEEN_FAMILY_PRICES_TO_USE_[$GradeToUse][$ContribToUse][1];
                                 }
                                 else
                                 {
                                     // Only price of the nursery for the lunch
                                     $BillWithoutMealAmount += $_CANTEEN_FAMILY_PRICES_TO_USE_[$GradeToUse][$ContribToUse][1];
                                 }
                             }
             ...
             $_NURSERY_FAMILY_PRICE_MODE_ = $_NURSERY_PRICES_MODES_[$FamilyMonthlyNurseryMode];
             $_NURSERY_FAMILY_PRICES_TO_USE_ = $_NURSERY_PRICES_TO_USE_[$FamilyMonthlyNurseryMode];
             ...
             {
                 switch($_NURSERY_FAMILY_PRICE_MODE_)
                 {
                     case 'M_NB_UP';
                         // Monthly prices. Each nursery registration must be paid with a unique price for the several timeslots
                         // (AM, PM, other timeslots)
                         foreach($ArrayNurseryRegistrations['NurseryRegistrationID'] as $nr => $NurseryRegistrationID)
                         {
                             ...
                             {
                                 // Price for AM
                                 $BillNurseryAmount += $_NURSERY_FAMILY_PRICES_TO_USE_[0];
                             ...
                             {
                                 // Price for PM
                                 $BillNurseryAmount += $_NURSERY_FAMILY_PRICES_TO_USE_[1];
                                 $BillNbNurseryRegistrations++;
                             }
                             ...
                             if ($iNbOtherTimeslots > 0)
                             {
                                 foreach($ArrayOtherTimeslotsPatterns as $ots => $CurrentPattern)
                                 {
                                     if ($ArrayNurseryRegistrations['NurseryRegistrationOtherTimeslots'][$nr] & $CurrentPattern)
                                     {
                                         ...
                                         if ((isset($_NURSERY_FAMILY_PRICES_TO_USE_['OtherTimeslots']))
                                             && (isset($_NURSERY_FAMILY_PRICES_TO_USE_['OtherTimeslots'][$ots])))
                                         {
                                             $BillNurseryAmount += $_NURSERY_FAMILY_PRICES_TO_USE_['OtherTimeslots'][$ots];
                                         }
                                     }
                                 }
                             }
                     ...
                     case 'M_NB_GR_UP';
                         // Monthly prices. Each nursery registration must be paid with a unique price for the several timeslots (AM, PM, other timeslots)
                         // in relation with child grade
                         foreach($ArrayNurseryRegistrations['NurseryRegistrationID'] as $nr => $NurseryRegistrationID)
                         {
                             $GradeToUse = 'G_'.$ArrayNurseryRegistrations['ChildGrade'][$nr];

                             if ($ArrayNurseryRegistrations['NurseryRegistrationForAM'][$nr] > 0)
                             {
                                 // Price for AM
                                 $BillNurseryAmount += $_NURSERY_FAMILY_PRICES_TO_USE_[$GradeToUse][0];
                                 $BillNbNurseryRegistrations++;
                             }
                             ...
                             {
                                 // Price for PM
                                 $BillNurseryAmount += $_NURSERY_FAMILY_PRICES_TO_USE_[$GradeToUse][1];
                                 $BillNbNurseryRegistrations++;
                             }
                             ...
                             if ($iNbOtherTimeslots > 0)
                             {
                                 foreach($ArrayOtherTimeslotsPatterns as $ots => $CurrentPattern)
                                 {
                                     if ($ArrayNurseryRegistrations['NurseryRegistrationOtherTimeslots'][$nr] & $CurrentPattern)
                                     {
                                         ...
                                         // Get price of the timeslot
                                         if ((isset($_NURSERY_FAMILY_PRICES_TO_USE_[$GradeToUse]['OtherTimeslots']))
                                             && (isset($_NURSERY_FAMILY_PRICES_TO_USE_[$GradeToUse]['OtherTimeslots'][$ots])))
                                         {
                                             $BillNurseryAmount += $_NURSERY_FAMILY_PRICES_TO_USE_[$GradeToUse]['OtherTimeslots'][$ots];
                                         }
                                     }
                                 }
                             }
                     ...
                     case 'M_PK_GR_CB':
                         // Monthly prices. Each price is a package in relation with child grade and monthy contribution mode of the family.
                         // We count the nursery registrations
                         $ArrayFamilyChildrenNR = array();
                         foreach($ArrayNurseryRegistrations['NurseryRegistrationID'] as $nr => $NurseryRegistrationID)
                         {
                            if (!isset($ArrayFamilyChildrenNR[$ArrayNurseryRegistrations['ChildID'][$nr]]))
                            {
                                $ArrayFamilyChildrenNR[$ArrayNurseryRegistrations['ChildID'][$nr]] = array(
                                                                                                           'AM' => 0,
                                                                                                           'PM' => 0,
                                                                                                           'OtherTimeslots' => 0
                                                                                                          );
                            ...
                            if ($ArrayNurseryRegistrations['NurseryRegistrationForAM'][$nr] > 0)
                            {
                                $ArrayFamilyChildrenNR[$ArrayNurseryRegistrations['ChildID'][$nr]]['AM']++;
                                $BillNbNurseryRegistrations++;
                            }

                            if ($ArrayNurseryRegistrations['NurseryRegistrationForPM'][$nr] > 0)
                            {
                                $ArrayFamilyChildrenNR[$ArrayNurseryRegistrations['ChildID'][$nr]]['PM']++;
                                $BillNbNurseryRegistrations++;
                            }
                            ...
                            if ($iNbOtherTimeslots > 0)
                            {
                                foreach($ArrayOtherTimeslotsPatterns as $ots => $CurrentPattern)
                                {
                                    if ($ArrayNurseryRegistrations['NurseryRegistrationOtherTimeslots'][$nr] & $CurrentPattern)
                                    {
                                        $ArrayFamilyChildrenNR[$ArrayNurseryRegistrations['ChildID'][$nr]]['OtherTimeslots']++;
                                        $BillNbNurseryRegistrations++;
                                    }
                                }
                            }
                         ...
                         // The family must pay nursery for each child
                         foreach($ArrayChildren['ChildID'] as $c => $CurrentChildID)
                         {
                             // We check if we must take into account this child (no desactivation date or desactivation date >= bill date
                             if ((is_null($ArrayChildren['ChildDesactivationDate'][$c]))
                                 || (strtotime($ArrayChildren['ChildDesactivationDate'][$c]) >= strtotime($BillForDate)))
                             {
                                 $GradeToUse = 'G_'.$ArrayChildren['ChildGrade'][$c];
                                 $ContribToUse = 'C_'.$ArrayFamilies['FamilyMonthlyContributionMode'][$f];

                                 // For AM and PM
                                 if (isset($_NURSERY_FAMILY_PRICES_TO_USE_[$GradeToUse][$ContribToUse]['0_1']))
                                 {
                                     $BillNurseryAmount += $_NURSERY_FAMILY_PRICES_TO_USE_[$GradeToUse][$ContribToUse]['0_1'];
                                 }
                                 else
                                 {
                                     // For AM
                                     $BillNurseryAmount += $_NURSERY_FAMILY_PRICES_TO_USE_[$GradeToUse][$ContribToUse][0];

                                     // For PM
                                     $BillNurseryAmount += $_NURSERY_FAMILY_PRICES_TO_USE_[$GradeToUse][$ContribToUse][1];
                                 }
                                 ...
                                 if ($iNbOtherTimeslots > 0)
                                 {
                                     ...
                                     {
                                         // Get price of the timeslot
                                         if ((isset($_NURSERY_FAMILY_PRICES_TO_USE_[$GradeToUse][$ContribToUse]['OtherTimeslots']))
                                             && (isset($_NURSERY_FAMILY_PRICES_TO_USE_[$GradeToUse][$ContribToUse]['OtherTimeslots'][$ots])))
                                         {
                                             $BillNurseryAmount += $_NURSERY_FAMILY_PRICES_TO_USE_[$GradeToUse][$ContribToUse]['OtherTimeslots'][$ots];
                                         }
                                     }
                                 }
                             }
                         ...
                         unset($ArrayFamilyChildrenNR);
                         break;

                     case 'A_PK_GR_CB':
                         // Annual prices. Each price is a package in relation with child grade and monthy contribution mode of the family.
                         // Prices are used only for the first bill of the school year !!!
                         ...
                         // We check if the generated bill is for the first month of the school year
                         if (date('m', strtotime($BillForDate)) == date('m', strtotime($CONF_SCHOOL_YEAR_START_DATES[$SchoolYear])))
                         {
                             // The family must pay nursery for each child
                             foreach($ArrayChildren['ChildID'] as $c => $CurrentChildID)
                             {
                                 // We check if we must take into account this child (no desactivation date or
                                 // desactivation date >= bill date
                                 if ((is_null($ArrayChildren['ChildDesactivationDate'][$c]))
                                     || (strtotime($ArrayChildren['ChildDesactivationDate'][$c]) >= strtotime($BillForDate)))
                                 {
                                     $GradeToUse = 'G_'.$ArrayChildren['ChildGrade'][$c];
             ...
             // We check if there are some nursery registrations have a delay
             if ((isset($ArrayNurseryRegistrations['NurseryRegistrationID'])) && (!empty($ArrayNurseryRegistrations['NurseryRegistrationID'])))
             {
                 foreach($ArrayNurseryRegistrations['NurseryRegistrationID'] as $nr => $NurseryRegistrationID)
                 {
