 ...
 *     - 2019-09-17 : v3.3. Display a waiting message when the page is loading
 *     - 2022-07-13 : v3.8. Allow to filter children by their grade and taken into account the new format of
 *                    $CONF_CANTEEN_DEFAULT_VIEW_TYPES variable (Type and Grade)
 *
 * @since 2012-01-28
 */
 ...
 {
     if (isset($CONF_CANTEEN_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]))
     {
         if (is_array($CONF_CANTEEN_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]))
         {
             // New format : Type and grade
             if (isset($CONF_CANTEEN_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]['Type']))
             {
                 $DefaultViewType = $CONF_CANTEEN_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]['Type'];
             }

             if ((isset($CONF_CANTEEN_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]['GradeGroup']))
                 && (isset($CONF_GRADES_GROUPS[$CONF_CANTEEN_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]['GradeGroup']])))
             {
                 // The default grades group exists
                 $DefaultGradeGroup = $CONF_CANTEEN_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]['GradeGroup'];
             }
         }
         else
         {
 ...
 if (!empty($_POST["lGradeGroup"]))
 {
     $GradeGroup = strip_tags($_POST["lGradeGroup"]);
 }
 else
 {
     if (!empty($_GET["lGradeGroup"]))
     {
         $GradeGroup = strip_tags($_GET["lGradeGroup"]);
     }
     else
     {
         $GradeGroup = $DefaultGradeGroup;  // Default grades group
     }
 }
 ...
 if (!empty($_POST["bSubmit"]))
 {
     ...
     switch($RestrictionAccess)
     {
         ...
         default:
             ...
             if (!empty($GradeGroup))
             {
                 $ArrayParamsChildrenFilters = array('ChildGrade' => $CONF_GRADES_GROUPS[$GradeGroup]);
             }

             $ArrayChildren = getChildrenListForCanteenPlanning($DbCon, $StartDate, $EndDate, "ChildID", FALSE, FALSE,
                                                                PLANNING_BETWEEN_DATES, $ArrayParamsChildrenFilters);
 ...
 switch($TypeView)
 {
     case PLANNING_WEEKS_VIEW:
         ...
         displayCanteenPlanningByWeeksForm($DbCon, "CanteenPlanning.php", $Week, $Year, $GradeGroup,
     ...
     default:
         ...
         // We display the planning
         displayCanteenPlanningByMonthForm($DbCon, "CanteenPlanning.php", $Month, $Year, $GradeGroup,
                                           $CONF_ACCESS_APPL_PAGES[FCT_CANTEEN_PLANNING], $CONF_CANTEEN_VIEWS_RESTRICTIONS);
         break;
 }
