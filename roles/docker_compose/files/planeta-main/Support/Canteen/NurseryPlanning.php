 ...
 *     - 2022-07-13 : v3.8. Allow to filter children by their grade
 *
 * @since 2012-02-14
 */
 ...
 {
     if (isset($CONF_NURSERY_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]))
     {
         if (is_array($CONF_NURSERY_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]))
         {
             // New format : Type and grade
             if (isset($CONF_NURSERY_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]['Type']))
             {
                 $DefaultViewType = $CONF_NURSERY_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]['Type'];
             }

             if ((isset($CONF_NURSERY_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]['GradeGroup']))
                 && (isset($CONF_GRADES_GROUPS[$CONF_NURSERY_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]['GradeGroup']])))
             {
                 // The default grades group exists
                 $DefaultGradeGroup = $CONF_NURSERY_DEFAULT_VIEW_TYPES[$_SESSION["SupportMemberStateID"]]['GradeGroup'];
             }
         }
         else
         {
 ...
 switch($TypeView)
 {
     case PLANNING_WEEKS_VIEW:
         ...
         displayNurseryPlanningByWeeksForm($DbCon, "NurseryPlanning.php", $Week, $Year, $GradeGroup,
                                           $CONF_ACCESS_APPL_PAGES[FCT_NURSERY_PLANNING], $CONF_NURSERY_VIEWS_RESTRICTIONS);
         break;
     ...
     default:
         ...
         // We display the planning
         displayNurseryPlanningByMonthForm($DbCon, "NurseryPlanning.php", $Month, $Year, $GradeGroup,
                                           $CONF_ACCESS_APPL_PAGES[FCT_NURSERY_PLANNING], $CONF_NURSERY_VIEWS_RESTRICTIONS);
         break;
 }
