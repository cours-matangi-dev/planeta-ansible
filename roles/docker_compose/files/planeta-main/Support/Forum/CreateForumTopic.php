...
/**
 * Support module : allow a supporter to create a new topic in a given forum category. The supporter must be logged
 * to create the topic.
 *
 ...
 */
 ...
 displayTitlePage($LANG_SUPPORT_CREATE_FORUM_TOPIC_PAGE_TITLE, 2);

 // The ID and the md5 crypted ID must be equal and the ID  must be > 0
 if (($Id > 0) && (md5($Id) == $CryptedID))
 {
     displayForumTopicForm($DbCon, 0, $Id, "ProcessNewForumTopic.php", $CONF_ACCESS_APPL_PAGES[FCT_FORUM]);
 }
 else
 {
