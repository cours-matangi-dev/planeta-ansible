...
/**
 * Support module : list messages of a given forum topic
 *
 ...
 */
 ...
 // To take into account the page of the forum topic messages to display
 if (!empty($_GET["Pg"]))
 {
 ...
 // To take into account the order by field to sort the table of the forum topic messages
 ...
 displayStyledLinkText($LANG_GO_TO_CONTENT, '#TopicMessagesList', 'Accessibility');
 ...
 displayTitlePage($LANG_SUPPORT_FORUM_TOPICS_MESSAGES_PAGE_TITLE, 2);

 // The ID and the md5 crypted ID must be equal
 if (md5($Id) == $CryptedID)
 {
     // Display the messages of this forum category
     displayForumTopicMessagesList($DbCon, $Id, "DisplayForumTopicMessages.php", $Page, "SortFct", $OrderBy,
