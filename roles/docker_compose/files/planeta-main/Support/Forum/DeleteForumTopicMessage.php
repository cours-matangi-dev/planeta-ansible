...
/**
 * Support module : delete the content of a forum message. The supporter must be logged to
 ...
 */
 ...
 // To take into account the crypted and no-crypted forum topic message ID
 // Crypted ID
 if (!empty($_GET["Cr"]))
 {
 ...
 if (isSet($_SESSION["SupportMemberID"]))
 {
     ...
     if (($Id != '') && (md5($Id) == $CryptedID))
     {
         ...
         // We erase the content of the message
         if ($bCanDelete)
         {
             // Yes, we can erase the content of the message
             $ForumMessageID = dbUpdateForumMessage($DbCon, $ForumMessageID, $ForumTopicID, NULL, $LANG_FORUM_MESSAGE_ERASED_MESSAGE,
                                                    date('Y-m-d H:i:s'));
             if ($ForumMessageID > 0)
             {
                 // Log event
                 ...
                 // The content message is deleted
                 $ConfirmationCaption = $LANG_CONFIRMATION;
                 $ConfirmationSentence = $LANG_CONFIRM_FORUM_TOPIC_MESSAGE_DELETED;
                 $ConfirmationStyle = "ConfirmationMsg";
             }
             else
             {
                 // ERROR : the content message isn't deleted
                 $ConfirmationCaption = $LANG_ERROR;
                 $ConfirmationSentence = $LANG_ERROR_DELETE_FORUM_MESSAGE;
                 $ConfirmationStyle = "ErrorMsg";
             }
         }
         else
         {
             // ERROR : the topic message ID is wrong
             $ConfirmationCaption = $LANG_ERROR;
