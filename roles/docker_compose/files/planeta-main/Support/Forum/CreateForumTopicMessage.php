...
/**
 * Support module : allow a supporter to create a new message for a given forum topic. The supporter must be logged
 * to create the forum message.
 *
 ...
 */
 ...
 // We check if its a "reply-to" message
 $ReplyToMessageID = NULL;
 if ((isset($_GET["Action"])) && (strTolower($_GET["Action"]) == 'replyto'))
 {
     // It's a "reply to" message
     if ((!empty($_GET["MCr"])) && (!empty($_GET["MId"])))
     {
         $MCrID = (string)strip_tags($_GET["MCr"]);
         $MId = (string)strip_tags($_GET["MId"]);
 ...
 displayStyledLinkText($LANG_GO_TO_CONTENT, '#MessageDetails', 'Accessibility');
 ...
 displayTitlePage($LANG_SUPPORT_CREATE_FORUM_TOPIC_MESSAGE_PAGE_TITLE, 2);

 // The ID and the md5 crypted ID must be equal and the ID  must be > 0
 if (($Id > 0) && (md5($Id) == $CryptedID))
 {
     displayForumTopicMessageForm($DbCon, 0, $Id, "ProcessNewForumTopicMessage.php", $CONF_ACCESS_APPL_PAGES[FCT_FORUM],
                                  $ReplyToMessageID);
 }
 else
 {
