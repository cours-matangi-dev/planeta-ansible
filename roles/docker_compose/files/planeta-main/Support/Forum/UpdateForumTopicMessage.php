...
/**
 * Support module : allow a supporter to update a given message of a forum topic. The supporter must be logged
 * to update the forum message.
 *
 ...
 */
 ...
 // To take into account the crypted and no-crypted forum message ID to update
 // Crypted ID
 if (!empty($_GET["Cr"]))
 {
 ...
 displayTitlePage($LANG_SUPPORT_UPDATE_FORUM_TOPIC_MESSAGE_PAGE_TITLE, 2);

 // The ID and the md5 crypted ID must be equal and the ID  must be > 0
 if (($Id > 0) && (md5($Id) == $CryptedID))
 {
     displayForumTopicMessageForm($DbCon, $Id, $ForumTopicID, "ProcessUpdateForumTopicMessage.php", $CONF_ACCESS_APPL_PAGES[FCT_FORUM], NULL);
 }
 else
 {
     openFrame($LANG_ERROR);
     displayStyledText($LANG_ERROR_NOT_VIEW_FORUM_MESSAGE, "ErrorMsg");
     closeFrame();
 }
