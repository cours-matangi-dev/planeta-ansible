...
/**
 * Support module : allow a supporter to subscribe to a given forum topic to receive notifications by e-mail
 * when new messages are posted by other supporters. The supporter must be logged to subscribe.
 *
 ...
 */
 ...
 // To take into account the crypted and no-crypted forum topic ID
 // Crypted ID
 if (!empty($_GET["Cr"]))
 {
 ...
 displayStyledLinkText($LANG_GO_TO_CONTENT, '#SubscribtionDetails', 'Accessibility');
 ...
 displayTitlePage($LANG_SUPPORT_SUBSCRIBE_TO_FORUM_TOPIC_PAGE_TITLE, 2);

 // The ID and the md5 crypted ID must be equal and the ID  must be > 0
 if (($Id > 0) && (md5($Id) == $CryptedID) && (isExistingForumTopic($DbCon, $Id)))
 {
     // Get infos about the topic
     $ForumTopicRecord = getTableRecordInfos($DbCon, "ForumTopics", $Id);

     openParagraph();
     echo $LANG_SUPPORT_SUBSCRIBE_TO_FORUM_TOPIC_PAGE_INTRODUCTION." <strong>".$ForumTopicRecord['ForumTopicTitle']."</strong>.";
     closeParagraph();

     displaySubscribeForumTopicForm($DbCon, $Id, "ProcessSubscribeForumTopic.php", $CONF_ACCESS_APPL_PAGES[FCT_FORUM]);
 }
 else
 {
