...
/**
 * Support module : list topics of a given forum category
 *
 ...
 */
 ...
 // To take into account the crypted and no-crypted forum category ID
 // Crypted ID
 if (!empty($_GET["Cr"]))
 {
 ...
 // Get the default language of the forum category
 $ForumCategoryDefaultLang = getForumCategoryDefaultLang($DbCon, $Id);
 ...
 // To take into account the page of the forum topics to display
 if (!empty($_GET["Pg"]))
 {
 ...
 // To take into account the order by field to sort the table of the forum topics
 ...
 $TabParams = array();
 $TabParams["ForumCategoryID"] = array($Id);
 $TabParams["All"] = TRUE;
 ...
 displayStyledLinkText($LANG_GO_TO_CONTENT, '#TopicsList', 'Accessibility');
 ...
 displayTitlePage($LANG_SUPPORT_FORUM_TOPICS_LIST_PAGE_TITLE, 2);

 openParagraph();
 echo $LANG_SUPPORT_FORUM_TOPICS_LIST_PAGE_INTRODUCTION." <strong>".getForumCategoryName($DbCon, $Id)."</strong>.";
 closeParagraph();

 // The ID and the md5 crypted ID must be equal
 if (md5($Id) == $CryptedID)
 {
     // Display the topics of this forum category
     displayForumCategoryTopicsList($DbCon, $TabParams, "DisplayForumCategoryTopics.php", $Page, "SortFct", $OrderBy,
                                    "DisplayForumTopicMessages.php", $CONF_ACCESS_APPL_PAGES[FCT_FORUM]);
 }
 else
 {
     openFrame($LANG_ERROR);
     displayStyledText($LANG_ERROR_NOT_VIEW_FORUM_CATEGORY, 'ErrorMsg');
     closeFrame();
 }
