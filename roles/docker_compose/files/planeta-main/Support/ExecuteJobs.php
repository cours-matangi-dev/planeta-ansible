 ...
 *     - 2017-10-10 : v3.1. Taken into account $CONF_JOBS_BEFORE_CHANGE_PLANNED_DATE
 *     - 2021-06-09 : v3.6. Taken into accoun MaxNbTries parameter in $CONF_JOBS_BEFORE_CHANGE_PLANNED_DATE
 *     - 2021-12-27 : v3.7. Patch getIntranetRootDirectoryHDD() for PHP8 and remove $CONF_URL_SUPPORT
 *
 * @since 2016-05-28
 */
     ...
     foreach($ArrayJobs['JobID'] as $j => $JobID)
     {
         ...
         switch($ArrayJobs['JobType'][$j])
         {
             case JOB_EMAIL:
                 ...
                 // We check if the nb of tries isn't too high
                 if ((isset($CONF_JOBS_BEFORE_CHANGE_PLANNED_DATE[JOB_EMAIL]))
                     && (isset($CONF_JOBS_BEFORE_CHANGE_PLANNED_DATE[JOB_EMAIL][MaxNbTries]))
                     && ($iNbTries > $CONF_JOBS_BEFORE_CHANGE_PLANNED_DATE[JOB_EMAIL][MaxNbTries]))
                 {
                     $bCanExecuteJob = FALSE;
                 }

                 if ($bCanExecuteJob)
                 {
                     // We can execute the job : the nb of tries isn't too high if a value is set
