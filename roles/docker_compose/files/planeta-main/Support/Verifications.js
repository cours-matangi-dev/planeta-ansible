 ...
 * @author Christophe Javouhey
 * @version 1.1
 *     - 2021-11-26 : v1.1. Taken into account the RequiredModesList parameter
 *
 * @since 2012-01-27
 *
 ...
 *                                       the payment mode is "Check"
 * @param RequiredModesList    String
 *
 * @return Boolean             TRUE if the payment form is correctly entered, FALSE otherwise
 */
 function VerificationPayment(DateMsgError, AmountMsgError, BankMsgError, CheckNbMsgError, RequiredModesList)
 {
     var ArrayRequiredModes = new Array();
     if (RequiredModesList != '')
     {
         ArrayRequiredModes = ListToArray(RequiredModesList);
     }
     ...
     {
         // We check if there are some payment modes which required a bank and a check number
         if (ArrayRequiredModes.length > 0)
         {
             if (In_Array(ArrayRequiredModes, document.forms[0].lPaymentMode.options[document.forms[0].lPaymentMode.selectedIndex].value))
             {
                 // Mode = check
...
/**
 * Function used to refresh the planning with the grade group selected in the dropdown list
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2022-07-13
 *
 * @param Value   Integer       Grade group selected
 */
 function onChangeSelectedGradeGroup(Value)
 {
     document.forms[0].submit();
 }
...
/**
 * Function used to delete content fo the Quill editor
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-04-23
...
/**
 * Function used to delete the expiration date of a topic forum
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-04-23
 */
 function ResetForumTopicExpirationDate()
 {
    if (document.forms[0].expirationDate)
    {
        document.forms[0].expirationDate.value = '';
    }
 }


/**
 * Function used to validate the new forum topic form
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-04-15
 *
 * @param TitleMsgError             String    Error message if the topic title field is empty
 * @param CategoryIDMsgError        String    Error message if no forum category is selected
 * @param ContentMsgError           String    Error message if the message content is empty
 * @param RankMsgError              String    Error message if a wrong topic rank value is entered
 * @param ExpirationDateMsgError    String    Error message if a wrong expiration date is entered
 *
 * @return Boolean                  TRUE if the new forum topic form is correctly entered, FALSE otherwise
 */
 function VerificationForumTopic(TitleMsgError, CategoryIDMsgError, ContentMsgError, RankMsgError, ExpirationDateMsgError)
 {
     if (document.forms[0].sTopicTitle.value == "")
     {
         alert(TitleMsgError);
         return false;
     }
     else if (document.forms[0].hidForumCategoryID.value <= 0)
     {
         alert(CategoryIDMsgError);
         return false;
     }
     else if ((document.forms[0].sTopicRank) && (document.forms[0].sTopicRank.value != "")
              && (isNaN(parseInt(document.forms[0].sTopicRank.value))))
     {
         alert(RankMsgError);
         return false;
     }
     else if (document.forms[0].expirationDate.value != "")
     {
         ExpirationDateDMY = ListSepToArray(document.forms[0].expirationDate.value, "/");
         ExpirationDate = new Date(ExpirationDateDMY[2], ExpirationDateDMY[1] - 1, ExpirationDateDMY[0]);

         Today = new Date();
         CurrentDate = new Date(Today.getFullYear(), Today.getMonth(), Today.getDate());

         // Compute the number of days between the expiration date and the current date
         DeadlineDelay = (ExpirationDate.getTime() - CurrentDate.getTime()) / 86400000;
         if (DeadlineDelay < 0)
         {
             alert(ExpirationDateMsgError);
             return false;
         }
     }

     // We copy the HTML content of the WYSIWYG editor in a hidden input
     var html = quill.root.innerHTML;
     var textContent = quill.root.textContent;
     if (textContent.trim() == '')
     {
         // Empty message or only with \n and whitespaces
         alert(ContentMsgError);
         return false;
     }
     else
     {
         var inputContent = document.getElementById('hidTopicContent');
...
/**
 * Function used to validate the new forum message form
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-04-17
 ...
 * @param ContentMsgError           String    Error message if the message content is empty
 *
 * @return Boolean                  TRUE if the new forum message form is correctly entered, FALSE otherwise
 */
 function VerificationForumTopicMessage(ContentMsgError)
 {
     // We copy the HTML content of the WYSIWYG editor in a hidden input
     var html = quill.root.innerHTML;
     var textContent = quill.root.textContent;
...
/**
 * Function used to validate the subscribtion forum topic form
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-04-22
 ...
 * @param EmailMsgError             String    Error message if the e-mail field is empty
 *
 * @return Boolean                  TRUE if the subscribtion to forum topic form is correctly entered,
 *                                  FALSE otherwise
 */
 function VerificationSubscribeForumTopic(EmailMsgError)
 {
...
/**
 * Function used to ask a question to the user with a yes/no answer
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-04-30
 *
 * @param Msg            String               Question to display
 *
 * @return Boolean       TRUE if the answer is "yes", FALSE otherwise
 */
...
/**
 * Function used to validate the forum category form
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-04-30
 ...
 * @param MandatoryMsgError       String    Error message if a mandatory field is empty or not valid
 *
 * @return Boolean                TRUE if the forum category form is correctly entered,
 *                                FALSE otherwise
 */
 function VerificationForumCategory(MandatoryMsgError)
 {
     if ((document.forms[0].sCategoryName.value == "") || (document.forms[0].sCategoryLang.value == ""))
     {
         alert(MandatoryMsgError);
         return false;
     }
...
/**
 * Function used to validate the forum category access form
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-04-30
 ...
 * @param MandatoryMsgError       String    Error message if a mandatory field is empty or not valid
 *
 * @return Boolean                TRUE if the forum category access form is correctly entered,
 *                                FALSE otherwise
 */
 function VerificationForumCategoryAccess(MandatoryMsgError)
 {
     if (document.forms[0].lSupportMemberStateID.options[document.forms[0].lSupportMemberStateID.selectedIndex].value == 0)
     {
         alert(MandatoryMsgError);
         return false;
     }
...
/**
 * Function used to validate the GDPR form
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-01
 ...
 * @param MandatoryMsgError       String    Error message if a mandatory field is empty or not valid
 *
 * @return Boolean                TRUE if the GDPR form is correctly entered, FALSE otherwise
 */
 function VerificationGDPR(MandatoryMsgError)
 {
     return true;
 }