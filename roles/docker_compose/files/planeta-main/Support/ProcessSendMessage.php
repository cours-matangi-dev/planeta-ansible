 ...
 *     - 2019-01-21 : v3.1. Taken into account internal alias "*@committee" to contact members of the committee
 *     - 2022-02-02 : v3.7. Taken into account support member state option to replace author lastname of the
 *                    message by the support member state name
 *
 * @since 2016-03-04
 */
 ...
 if (!empty($_POST["bSubmit"]))
 {
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         // We check if the Support member state option about send message is activated for this support member state
         if ($CONF_SUPPORT_NB_SUPPORT_MEMBER_STATE_OPTIONS > 0)
         {
             $SupportMemberStateRecord = getTableRecordInfos($DbCon, "SupportMembersStates", $_SESSION["SupportMemberStateID"]);
             if ($SupportMemberStateRecord['SupportMemberStateOptions'] & pow(2,SMS_OPT_SEND_MESSAGE_AUTHOR))
             {
                 $sAuthor = $SupportMemberStateRecord['SupportMemberStateName'];
             }
         }
