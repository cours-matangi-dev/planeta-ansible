...
/**
 * Support module : mangement of data for GDPR
 *
 ...
 */
 ...
 // To take into account the page of the data about GDPR to display
 if (!empty($_GET["Pg"]))
 {
 ...
 // To take into account the order by field to sort the table of the data about GDPR
 ...
 {
     foreach($CONF_GDPR_NB_YEARS_DATA_DELAY as $TableName => $CurrentNbYears)
     {
         // We compute the date for which data must be deleted
         if ($CurrentNbYears > 0)
         {
             $TooOldDate = getSchoolYearStartDate($CurrentSchoolYear - $CurrentNbYears);
             $TabParams[$TableName] = array('<', $TooOldDate);
         }
     }

     if ((isset($CONF_GDPR_ANONYMIZED_SUPPORT_MEMBER_ID)) && ($CONF_GDPR_ANONYMIZED_SUPPORT_MEMBER_ID > 0))
     {
         $TabParams['AnonymizedSupportMemberID'] = $CONF_GDPR_ANONYMIZED_SUPPORT_MEMBER_ID;
     }
 }
 ...
 displayStyledLinkText($LANG_GO_TO_CONTENT, '#GDPRList', 'Accessibility');
 ...
 if (isSet($_SESSION["SupportMemberID"]))
 {
     ...
     displaySupportMemberContextualMenu("admin", 1, Admin_GDPR);
 ...
 displayTitlePage($LANG_SUPPORT_ADMIN_GDPR_MANAGEMENT_PAGE_TITLE, 2);

 openParagraph();
 echo $LANG_SUPPORT_ADMIN_GDPR_MANAGEMENT_PAGE_INTRODUCTION;
 closeParagraph();

 displaySearchGDPRDataForm($DbCon, $TabParams, "GDPRManagement.php", $Page, "SortFct", $OrderBy,
