...
/**
 * Support module : process the creation of a new forum category. The supporter must be logged.
 *
 ...
 */
 ...
 if (!empty($_POST["bSubmit"]))
 {
     if ((isSet($_SESSION["SupportMemberID"])) && (isAdmin()))
     {
         ...
         if ($ContinueProcess)
         {
             $ForumCategoryID = dbAddForumCategory($DbCon, $CategoryName, $CategoryLang, $Description);

             if ($ForumCategoryID != 0)
             {
                 // The forum category is added
             ...
             {
                 // The forum category can't be added
                 $ConfirmationCaption = $LANG_ERROR;
                 $ConfirmationSentence = $LANG_ERROR_ADD_RECORD;
                 $ConfirmationStyle = "ErrorMsg";
                 $UrlParameters = "AddForumCategory.php?$QUERY_STRING"; // For the redirection
             }
         }
         else
         {
 ...
 {
     // The supporter doesn't come from the AddForumCategory.php page
     $ConfirmationCaption = $LANG_ERROR;
