...
/**
 * Support module : process the update of a forum category access. The supporter must be logged.
 *
 ...
 */
 ...
 if (!empty($_POST["bSubmit"]))
 {
     if ((isSet($_SESSION["SupportMemberID"])) && (isAdmin()))
     {
         ...
         if ($ContinueProcess)
         {
             $ForumCategoryAccessID = dbUpdateForumCategoryAccess($DbCon, $ForumCategoryAccessID, $ForumCategoryID,
                                                                  $SupportMemberStateID, $CategoryAccess);
             if ($ForumCategoryAccessID != 0)
             {
                 // The forum category access is updated
                 $ConfirmationCaption = $LANG_CONFIRMATION;
                 $ConfirmationSentence = $LANG_CONFIRM_RECORD_UPDATED;
                 $ConfirmationStyle = "ConfirmationMsg";
                 $UrlParameters = "Cr=".md5($ForumCategoryAccessID)."&Id=$ForumCategoryAccessID"; // For the redirection
             }
             else
             {
                 // The forum category access can't be updated
 ...
 {
     // The supporter doesn't come from the UpdateForumCategoryAccess.php page
     $ConfirmationCaption = $LANG_ERROR;
 ...
 initGraphicInterface(
                      ...
                      'WhitePage',
                      "Redirection('".$CONF_ROOT_DIRECTORY."Support/Admin/UpdateForumCategoryAccess.php?$UrlParameters', $CONF_TIME_LAG)"
                     );
