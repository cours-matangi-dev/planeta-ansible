 ...
 * @author Christophe Javouhey
 * @version 1.1
 *     - 2022-07-13 : v1.1. Add filter by grade groups
 *
 * @since 2013-01-25
 *
 ...
 * @param $GradeGroup           String                Selected grade group to fileter children on their grade
 ...
 */
 function displayNurseryPlanningByMonthForm($DbConnection, $ProcessFormPage, $Month, $Year, $GradeGroup = '', $AccessRules = array(), $ViewsRestrictions = array())
 {
     ...
     displayNurseryPlanningForm($DbConnection, $ProcessFormPage, $StartDate, $EndDate, $SelectedDate, PLANNING_MONTH_VIEW,
                                $GradeGroup, $AccessRules, $ViewsRestrictions);
 }
 ...
 *                                                    allowed to view some nursery registratrions
 */
 function displayNurseryPlanningByWeeksForm($DbConnection, $ProcessFormPage, $Week, $Year, $GradeGroup = '', $AccessRules = array(), $ViewsRestrictions = array())
 {
     // Compute start date and end date of the month
 ...
 *     - 2019-09-17 : v4.1. Display a waiting message when the page is loading
 *     - 2020-01-22 : v4.2. Taken into account CONF_NURSERY_REGISTER_DELAY_BEFORE_PLANNING_REGISTRATION and
 *                    $CONF_NURSERY_OTHER_TIMESLOTS
 *     - 2021-01-04 : v4.3. Remove {} and use substr() to extract 1 character from a string (for PHP 7.4)
 *     - 2022-07-13 : v4.4. Add filter by grade groups
 *
 * @since 2012-02-14
 *
 * @param $DbConnection         DB object             Object of the opened database connection
 ...
 */
 function displayNurseryPlanningForm($DbConnection, $ProcessFormPage, $StartDate, $EndDate, $SelectedDate, $ViewType = PLANNING_MONTH_VIEW, $GradeGroup = '', $AccessRules = array(), $ViewsRestrictions = array())
 {
     ...
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         if (in_array($cUserAccess, array(FCT_ACT_CREATE, FCT_ACT_UPDATE, FCT_ACT_READ_ONLY, FCT_ACT_PARTIAL_READ_ONLY)))
         {
             ...
             // <<< Grade groups SELECTFIELD >>>
             $ArrayGradeGroupsNames = array_merge(array(''), array_keys($GLOBALS['CONF_GRADES_GROUPS']));
             $ArrayGradeGroupsLabels = array_merge(array('-'), array_keys($GLOBALS['CONF_GRADES_GROUPS']));
             $GradeGroupsList = generateSelectField("lGradeGroup", $ArrayGradeGroupsNames, $ArrayGradeGroupsLabels, $GradeGroup,
                                                    "onChangeSelectedGradeGroup(this.value)");
             ...
             switch($ViewType)
             {
                 case PLANNING_WEEKS_VIEW:
                     ...
                     $PlanningViewTypeCaption = ucfirst(substr($GLOBALS["LANG_WEEK"], 0, 1))."$Week-$YearOfWeek";
                     ...
                     displayStyledLinkText($GLOBALS["LANG_PREVIOUS"],
                                           "$ProcessFormPage?lView=".PLANNING_WEEKS_VIEW."&amp;lWeek=$PreviousWeek&amp;lYear=$PreviousYear&amp;lGradeGroup=$GradeGroup",
                                           'prev', $GLOBALS["LANG_PREVIOUS"]);

                     // Display the year list to change the planning to display
                     echo " $ViewsList $WeeksList $YearsList $GradeGroupsList ";
                     ...
                     displayStyledLinkText($GLOBALS["LANG_NEXT"],
                                           "$ProcessFormPage?lView=".PLANNING_WEEKS_VIEW."&amp;lWeek=$NextWeek&amp;lYear=$NextYear&amp;lGradeGroup=$GradeGroup",
                 ...
                 default:
                     ...
                     displayStyledLinkText($GLOBALS["LANG_PREVIOUS"],
                                           "$ProcessFormPage?lView=".PLANNING_MONTH_VIEW."&amp;lMonth=$PreviousMonth&amp;lYear=$PreviousYear&amp;lGradeGroup=$GradeGroup",
                                           'prev', $GLOBALS["LANG_PREVIOUS"]);

                     // Display the year list to change the planning to display
                     echo " $ViewsList $MonthsList $YearsList $GradeGroupsList ";
                     ...
                     displayStyledLinkText($GLOBALS["LANG_NEXT"],
                                           "$ProcessFormPage?lView=".PLANNING_MONTH_VIEW."&amp;lMonth=$NextMonth&amp;lYear=$NextYear&amp;lGradeGroup=$GradeGroup",
                                           'next', $GLOBALS["LANG_NEXT"]);
             }
             ...
             switch($RestrictionAccess)
             {
                 ...
                 default:
                     ...
                     $ArrayParamsChildrenFilters = array();
                     if (!empty($GradeGroup))
                     {
                         $ArrayParamsChildrenFilters = array('ChildGrade' => $GLOBALS['CONF_GRADES_GROUPS'][$GradeGroup]);
                     }

                     $ArrayChildren = getChildrenListForNurseryPlanning($DbConnection, $StartDate, $EndDate, "ChildClass, FamilyLastname",
                                                                        FALSE, PLANNING_BETWEEN_DATES, $ArrayParamsChildrenFilters);
