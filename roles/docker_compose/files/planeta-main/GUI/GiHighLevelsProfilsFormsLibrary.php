 ...
 * @author Christophe Javouhey
 * @version 1.2
 *     - 2016-12-02 : v1.1. Add the SupportMemberID in a hidden input field to allow admin to
 *                    update a login/password of another support member
 *     - 2021-11-25 : v1.2. Add "*" to all 3 fields
 *
 * @since 2012-01-13
 *
 * @param $DbConnection         DB object             Object of the opened database connection
 ...
 function displayDetailsSupportMemberStateForm($DbConnection, $SupportMemberStateID, $ProcessFormPage, $AccessRules = array())
 {
     ...
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         if (in_array($cUserAccess, array(FCT_ACT_CREATE, FCT_ACT_UPDATE, FCT_ACT_READ_ONLY, FCT_ACT_PARTIAL_READ_ONLY)))
         {
             ...
             // <<< SupportMemberStateOptions INPUTFIELDS >>>
             switch($cUserAccess)
             {
                 case FCT_ACT_READ_ONLY:
                 case FCT_ACT_PARTIAL_READ_ONLY:
                     $sStateOptions = "";
                     for($Opt = 0; $Opt < $GLOBALS['CONF_SUPPORT_NB_SUPPORT_MEMBER_STATE_OPTIONS']; $Opt++)
                     {
                         if (isset($GLOBALS['LANG_USER_STATUS_OPTIONS_LABEL_'.$Opt]))
                         {
                             // The option is activated ?
                             if ($SupportMemberStateRecord["SupportMemberStateOptions"] & pow(2, $Opt))
                             {
                                 $sStateOptions .= $GLOBALS['LANG_USER_STATUS_OPTIONS_LABEL_'.$Opt].generateBR(1);
                             }
                         }
                     }
                     break;
                 ...
                 case FCT_ACT_UPDATE:
                     ...
                     for($Opt = 0; $Opt < $GLOBALS['CONF_SUPPORT_NB_SUPPORT_MEMBER_STATE_OPTIONS']; $Opt++)
                     {
                         if (isset($GLOBALS['LANG_USER_STATUS_OPTIONS_LABEL_'.$Opt]))
                         {
                             ...
                             $bChecked = $SupportMemberStateRecord["SupportMemberStateOptions"] & pow(2, $Opt);

                             $sStateOptions .= generateInputField("chkSupportMemberStateOption_$Opt", "checkbox", "", "",
                                                                  $GLOBALS['LANG_USER_STATUS_OPTIONS_TIP_'.$Opt], 1,
                                                                  FALSE, $bChecked)." ".$GLOBALS['LANG_USER_STATUS_OPTIONS_LABEL_'.$Opt]
                                                                  .generateBR(1);
                         }
                     }
                     break;
             ...
             echo "<tr>\n\t<td class=\"Label\">".$GLOBALS['LANG_USER_STATUS_OPTIONS']."</td><td class=\"Value\">$sStateOptions</td>\n</tr>\n";
             echo "</table>\n";
