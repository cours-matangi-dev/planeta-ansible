 ...
 * @author Christophe Javouhey
 * @version 2.2
 *     - 2013-10-15 : v1.1. Display a progress bar for each child event in relation with the registered families
 *     - 2014-02-25 : v1.2. Display an achor to go directly to content
 *     - 2015-10-06 : v1.3. Taken into account the FamilyID field of SupportMembers table and patch a bug
 *                    to display start time and end time
 *     - 2015-12-09 : v1.4. Display an icon when the registration on an event isn't valided
 *     - 2016-06-20 : v1.5. Remove htmlspecialchars() function
 *     - 2016-09-09 : v1.6. Patch some display bugs about start time and end time, display a button
 *                    to add a new town, patch a bug about $EndDate
 *     - 2019-06-14 : v1.7. Taken into account $CONF_COOP_EVENT_USE_RANDOM_AUTO_FAMILIES_REGISTRATIONS
 *                    and display the number of registered families
 *     - 2019-11-08 : v1.8. Calendar has a unique callback function (calendarCallback) and has input field
 *                    and $CONF_LANG in parameter, allow to link uploaded files to the event
 *     - 2020-01-22 : v1.9. Add button to check/uncheck all checkboxes of the communication function
 *     - 2021-01-25 : v2.0. Add placeholder for start time and end time fields
 *     - 2021-05-07 : v2.1. Taken into account the Stats table when the number of registrations is 0
 *                    because of the GDRP
 *     - 2021-09-10 : v2.2. Patch a bug about "if ($EventID == 0)" for PHP8
 *
 * @since 2013-04-04
 *
         ...
         if (in_array($cUserAccess, array(FCT_ACT_CREATE, FCT_ACT_UPDATE, FCT_ACT_READ_ONLY, FCT_ACT_PARTIAL_READ_ONLY)))
         {
             ...
             if ($EventID > 0)
             {
                     ...
                     if ((isset($ArrayChildEvents["EventID"])) && (count($ArrayChildEvents["EventID"]) > 0))
                     {
                         foreach($ArrayChildEvents["EventID"] as $i => $CurrentID)
                         {
                             if ($CurrentID != $EventID)
                             {
                                 ...
                                 if ($iNbRegistrations == 0)
                                 {
                                     // No registrations maybe because of GDRP. We check if the Stats table contains the nb of registrations for this sub-event
                                     $ArrayStats = dbSearchStat($DbConnection, array(
                                                                                     'StatType' => STAT_TYPE_NB_EVENT_REGISTRATIONS,
                                                                                     'StatSubType' => $CurrentID
                                                                                    ), "StatID", 1, 0);

                                     if ((isset($ArrayStats["StatID"])) && (count($ArrayStats["StatID"]) > 0))
                                     {
                                         $iNbRegistrations = (integer)$ArrayStats['StatValue'][0];  // Cast because the stat value is a float
                                     }
                 ...
                 if ($bClosed)
                 {
                     ...
                     {
                         // No registrations maybe because of GDRP. We check if the Stats table contains the nb of registrations for this event
                         $ArrayStats = dbSearchStat($DbConnection, array(
                                                                         'StatType' => STAT_TYPE_NB_EVENT_REGISTRATIONS,
                                                                         'StatSubType' => $EventID
                                                                        ), "StatID", 1, 0);
                         ...
                         if ((isset($ArrayStats["StatID"])) && (count($ArrayStats["StatID"]) > 0))
                         {
                             $iNbFamiliesRegistrations = (integer)$ArrayStats['StatValue'][0];  // cast because the stat value is a float
                         }
 ...
 * @author Christophe Javouhey
 * @version 1.3
 *     - 2014-02-25 : v1.1. Display an achor to go directly to content
 *     - 2015-10-06 : v1.2. Taken into account the FamilyID field of SupportMembers table
 *     - 2021-05-10 : v1.3. Taken into account the Stats table when the number of registrations is 0
 *                    because of the GDRP
 *
 * @since 2013-04-10
 *
             ...
             if ($NbTabParams > 0)
             {
                 ...
                 if ($NbRecords > 0)
                 {
                     ...
                     foreach($ArrayRecords["EventID"] as $i => $CurrentValue)
                     {
                         ...
                         if ((empty($sNbRegistrations)) && (empty($iTotalNbRegistrations)))
                         {
                             // No registrations maybe because of GDRP. We check if the Stats table contains the nb of registrations for this sub-event
                             $ArrayStats = dbSearchStat($DbConnection, array(
                                                                             'StatType' => STAT_TYPE_NB_EVENT_REGISTRATIONS,
                                                                             'StatSubType' => $CurrentValue
                                                                            ), "StatID", 1, 0);

                             if ((isset($ArrayStats["StatID"])) && (count($ArrayStats["StatID"]) > 0))
                             {
                                 $iTotalNbRegistrations = (integer)$ArrayStats['StatValue'][0];  // Cast because the stat value is a float
