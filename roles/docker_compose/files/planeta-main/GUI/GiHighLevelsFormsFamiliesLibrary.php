 ...
 *     - 2016-05-10 : v3.0. Taken into account of the new values in $CONF_MONTHLY_CONTRIBUTION_MODES (about coefficients
 *                    of families), add a button to add new towns and remove htmlspecialchars() function and
 *                    add a button to delete a payment
 *     - 2017-09-21 : v3.1. Taken into account FCT_ACT_UPDATE_OLD_USER access for old families. They can update
 ...
 *     - 2019-05-10 : v3.2. Display documents approvals for the family and display users profils linked to the family,
 *                    taken into account the FamilyAnnualContributionBalance field
 *     - 2019-11-08 : v3.3. Calendar has a unique callback function (calendarCallback) and has input field and $CONF_LANG
 *                    in parameter, round the remaining amount to pay on a bill
 *     - 2020-07-01 : v3.4. Add HTML ID for some areas in the form
 *     - 2021-05-18 : v3.5. Add 2 picture fields to add a picture for each e-mail address
 *     - 2021-09-10 : v3.6. Patch a bug about "if ($FamilyID == 0)" for PHP8
 *     - 2022-06-06 : v3.7. Taken into account FamilyMonthlyNurseryMode field, allow to edit a bill ($CONF_BILLS_ALLOW_EDIT_BILLS)
 *                    and display workgroups registrations.
 *
 * @since 2012-01-16
 *
 ...
 */
 function displayDetailsFamilyForm($DbConnection, $FamilyID, $ProcessFormPage, $AccessRules = array())
 {
     ...
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         {
             // Open a form
             openForm("FormDetailsFamily", "post", "$ProcessFormPage?".$GLOBALS["QUERY_STRING"], "multipart/form-data",
                      "VerificationFamily('".$GLOBALS["LANG_ERROR_JS_FAMILY_LASTNAME"]."', '".$GLOBALS["LANG_ERROR_JS_FAMILY_MAIN_EMAIL"]."', '"
                                          .$GLOBALS["LANG_ERROR_JS_FAMILY_SECOND_EMAIL"]."', '".$GLOBALS["LANG_ERROR_JS_FAMILY_WRONG_NB_MEMBERS"]
                                          ."', '".$GLOBALS["LANG_ERROR_JS_TOWN"]."')");
                         ...
                         case FCT_ACT_UPDATE:
                             ...
                             if ((isset($ArrayBills["BillID"])) && (count($ArrayBills["BillID"]) > 0))
                             {
                                 ...
                                 foreach($ArrayBills["BillID"] as $i => $CurrentID)
                                 {
                                     ...
                                     $sBillAccess = '';
                                     if ($GLOBALS['CONF_BILLS_ALLOW_EDIT_BILLS'])
                                     {
                                         $sBillAccess .= generateStyledPictureHyperlink($GLOBALS['CONF_EDIT_BILL_ICON'], 'UpdateBill.php?Cr='
                                                                                        .md5($CurrentID)."&amp;Id=$CurrentID", $GLOBALS['LANG_FAMILY_BILL_EDIT_TIP'],
                                                                                        '', '_blank', '').' ';
                                     }

                                     $sBillAccess .= generateCryptedHyperlink($BillCaption, $CurrentID, 'ViewBill.php', $GLOBALS["LANG_VIEW_DETAILS_INSTRUCTIONS"],
                                                                              '', '_blank');
                                     $TabBillsData[0][] = $sBillAccess;
             ...
             // <<< FamilyMainPicture INPUTFIELD >>>
             if ($bClosed)
             {
                 $MainPicture = '';
                 if ((!empty($FamilyRecord["FamilyMainPicture"]))
                     && (file_exists($GLOBALS['CONF_UPLOAD_FAMILY_PICTURE_FILES_DIRECTORY'].$FamilyRecord["FamilyMainPicture"])))
                 {
                     // We display the picture
                     $MainPicture = generateStyledPicture($GLOBALS['CONF_UPLOAD_FAMILY_PICTURE_FILES_DIRECTORY'].$FamilyRecord["FamilyMainPicture"],
                                                          '', 'LittlePicture');
                 }
             }
             else
             {
                 switch($cUserAccess)
                 {
                     ...
                     case FCT_ACT_UPDATE:
                     case FCT_ACT_PARTIAL_READ_ONLY:
                         $MainPicture = generateInputField("fMainPicture", "file", "", "50", $GLOBALS["LANG_FAMILY_MAIN_PICTURE_TIP"], "");
                         if ((!empty($FamilyRecord["FamilyMainPicture"]))
                             && (file_exists($GLOBALS['CONF_UPLOAD_FAMILY_PICTURE_FILES_DIRECTORY_HDD'].$FamilyRecord["FamilyMainPicture"])))
                         {
                             ...
                             $MainPicture .= ' '.generateStyledPictureHyperlink($GLOBALS["CONF_DELETE_ICON"],
                                                                                "DeleteFamilyPicture.php?Cr=".md5($FamilyID)."&amp;Id=$FamilyID&amp;PCr=".md5(1)."&amp;PId=1",
                                                                                $GLOBALS["LANG_DELETE"], 'Affectation');
                         }
                         break;
                 }
             ...
             // <<< FamilySecondPicture INPUTFIELD >>>
             if ($bClosed)
             {
                 $SecondPicture = '';
                 if ((!empty($FamilyRecord["FamilySecondPicture"]))
                     && (file_exists($GLOBALS['CONF_UPLOAD_FAMILY_PICTURE_FILES_DIRECTORY'].$FamilyRecord["FamilySecondPicture"])))
                 {
                     // We display the picture
                     $SecondPicture = generateStyledPicture($GLOBALS['CONF_UPLOAD_FAMILY_PICTURE_FILES_DIRECTORY'].$FamilyRecord["FamilySecondPicture"],
                                                            '', 'LittlePicture');
                 }
             }
             else
             {
                 switch($cUserAccess)
                 {
                     ...
                     case FCT_ACT_UPDATE:
                     case FCT_ACT_PARTIAL_READ_ONLY:
                         $SecondPicture = generateInputField("fSecondPicture", "file", "", "50", $GLOBALS["LANG_FAMILY_SECOND_PICTURE_TIP"], "");
                             ...
                             && (file_exists($GLOBALS['CONF_UPLOAD_FAMILY_PICTURE_FILES_DIRECTORY_HDD'].$FamilyRecord["FamilySecondPicture"])))
                         {
                             ...
                             // Add the delete button
                             $SecondPicture .= ' '.generateStyledPictureHyperlink($GLOBALS["CONF_DELETE_ICON"],
                                                                                  "DeleteFamilyPicture.php?Cr=".md5($FamilyID)."&amp;Id=$FamilyID&amp;PCr=".md5(2)."&amp;PId=2",
                                                                                  $GLOBALS["LANG_DELETE"], 'Affectation');
                         }
                         break;
                 }
             ...
             // <<< FamilyMonthlyNurseryMode SELECTFIELD >>>
             if ($bClosed)
             {
                 // We get the last school year for this family
                 $FamilyLastSchoolYear = getSchoolYear('2010-01-01');
                 foreach($ArrayChildren['ChildDesactivationDate'] as $c => $CurrentChildDesactivationDate)
                 {
                     if (!empty($CurrentChildDesactivationDate))
                     {
                         if (getSchoolYear($CurrentChildDesactivationDate) > $FamilyLastSchoolYear)
                         {
                             $FamilyLastSchoolYear = getSchoolYear($CurrentChildDesactivationDate);
                         }
                     }
                 }

                 // Next, we get the monthly nursery mode label
                 // We get the monthly nursery modes available for the last school year of the family
                 if ((isset($GLOBALS['CONF_NURSERY_PRICES'][$FamilyLastSchoolYear]))
                     && (!empty($GLOBALS['CONF_NURSERY_PRICES'][$FamilyLastSchoolYear])))
                 {
                     // We detect the nursery mode
                     $ArrayTmpNurseryModes = array_keys($GLOBALS['CONF_NURSERY_PRICES'][$FamilyLastSchoolYear]);
                     if (isInteger($ArrayTmpNurseryModes[0]))
                     {
                         // Default mode (AM, PM and other timeslots prices)
                         $AvailableMonthlyNurseryModes = array(0 => $GLOBALS['LANG_FAMILY_MONTHLY_NURSERY_DEFAULT_MODE']);
                     }
                     else
                     {
                         foreach($ArrayTmpNurseryModes as $tnm => $CurrentNurseryMode)
                         {
                             $AvailableMonthlyNurseryModes[$tnm] = $CurrentNurseryMode;
                         }
                     }
                 }
                 else
                 ...
                 $MonthlyNurseryMode = $AvailableMonthlyNurseryModes[$FamilyRecord['FamilyMonthlyNurseryMode']];
                 unset($ArrayTmpNurseryModes);
             }
             else
             {
                 // We get the monthly nursery modes available for the current school year
                 if ((isset($GLOBALS['CONF_NURSERY_PRICES'][$CurrentSchoolYear]))
                     && (!empty($GLOBALS['CONF_NURSERY_PRICES'][$CurrentSchoolYear])))
                 {
                     // We detect the nursery mode
                     $ArrayTmpNurseryModes = array_keys($GLOBALS['CONF_NURSERY_PRICES'][$CurrentSchoolYear]);
                     if (isInteger($ArrayTmpNurseryModes[0]))
                     {
                         // Default mode (AM, PM and other timeslots prices)
                         $AvailableMonthlyNurseryModes = array(0 => $GLOBALS['LANG_FAMILY_MONTHLY_NURSERY_DEFAULT_MODE']);
                 ...
                 switch($cUserAccess)
                 {
                     ...
                     case FCT_ACT_CREATE:
                     case FCT_ACT_UPDATE:
                         // Generate the list of monthly nursery modes only available for the current school year
                         $ArrayMonthlyNurseryModes = array_keys($AvailableMonthlyNurseryModes);
                         $ArrayMonthlyNurseryLabels = array_values($AvailableMonthlyNurseryModes);

                         $MonthlyNurseryMode = generateSelectField("lMonthlyNurseryMode", $ArrayMonthlyNurseryModes,
                                                                   $ArrayMonthlyNurseryLabels,
                                                                   $FamilyRecord['FamilyMonthlyNurseryMode']);
                         break;
                 }
             }
             ...
             // Get workgroups registrations linked to the family
             $WorkgroupRegistrationsList = "&nbsp;";
             $ArrayWorkgroupRegistrations = dbSearchWorkGroupRegistration($DbConnection, array('FamilyID' => array($FamilyID)),
                                                                          "WorkGroupRegistrationLastname, WorkGroupRegistrationFirstname", 1, 0);
             if ((isset($ArrayWorkgroupRegistrations['WorkGroupRegistrationID'])) && (!empty($ArrayWorkgroupRegistrations['WorkGroupRegistrationID'])))
             {
                 $WGRStyle = "";
                 if ($bClosed)
                 {
                     // No workgroup registration for a desactivated family !
                     $WGRStyle = "Desactivated";
                 }

                 $WorkgroupRegistrationsList = "<ul>\n";

                 foreach($ArrayWorkgroupRegistrations['WorkGroupRegistrationID'] as $wgr => $CurrentID)
                 {
                     $WorkgroupRegistrationsList .= "<li>".generateStyledText($ArrayWorkgroupRegistrations['WorkGroupRegistrationLastname'][$wgr]
                                                          .' '.$ArrayWorkgroupRegistrations['WorkGroupRegistrationFirstname'][$wgr]
                                                          ." (".$ArrayWorkgroupRegistrations['WorkGroupName'][$wgr].")", $WGRStyle)."</li>\n";
                 }
             ...
             echo "<tr>\n\t<td class=\"Label\">".$GLOBALS["LANG_FAMILY_MAIN_PICTURE"]."</td><td class=\"Value\" colspan=\"3\">$MainPicture</td>\n</tr>\n";
             echo "<tr>\n\t<td class=\"Label\">".$GLOBALS["LANG_FAMILY_SECOND_EMAIL"]."</td><td class=\"Value\" colspan=\"3\">$SecondEmail</td>\n</tr>\n";
             echo "<tr>\n\t<td class=\"Label\">".$GLOBALS["LANG_FAMILY_SECOND_PICTURE"]."</td><td class=\"Value\" colspan=\"3\">$SecondPicture</td>\n</tr>\n";
             ...
             echo "<tr>\n\t<td class=\"Label\">".$GLOBALS["LANG_FAMILY_MONTHLY_NURSERY_MODE"]."</td><td class=\"Value\">$MonthlyNurseryMode</td><td class=\"Label\">&nbsp;</td><td class=\"Value\">&nbsp;</td>\n</tr>\n";
             echo "<tr>\n\t<td class=\"Label\">".$GLOBALS["LANG_USER_STATUS_S"]."</td><td class=\"Value\">$SupportMembersList</td><td class=\"Label\">".$GLOBALS["LANG_WORKGROUPS"]."</td><td class=\"Value\">$WorkgroupRegistrationsList</td>\n</tr>\n";
