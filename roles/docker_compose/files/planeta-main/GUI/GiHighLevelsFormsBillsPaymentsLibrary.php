 ...
 * @author Christophe Javouhey
 * @version 2.4
 *     - 2013-01-21 : v1.1. Allow to add/remove one or several bills to the payment, not allow
 *                    to change the type of payment after it's created and taken into account
 *                    the new field PaymentReceiptDate
 *     - 2013-11-22 : v1.2. Display the amount of the payment affected to each linked bill
 *     - 2013-12-16 : v1.3. Taken into account the "PaymentUsedAmount" field
 *     - 2015-01-19 : v1.4. Display the not used amount and the total of paid amounts of bills, display
 ...
 *     - 2016-06-20 : v1.5. Remove htmlspecialchars() function
 *     - 2016-10-12 : v2.0. Allow to force the part amount of a payment linked to a bill (CONF_PAYMENTS_MANUAL_BILL_PART_AMOUNT)
 *     - 2019-11-08 : v2.1. Calendar has a unique callback function (calendarCallback) and has input field and $CONF_LANG
 *                    in parameter. The supporter can't select another payment type (select field removed, hidden field instead !)
 *     - 2020-01-30 : v2.2. Display the remaining amount of a not paid bill in the list to link bills to a payment
 *     - 2020-07-01 : v2.3. Taken into account $CONF_DEFAULT_VALUES_SET to set default values
 *     - 2021-11-26 : v2.4. Taken into account dynamically $CONF_PAYMENTS_MODES_BANK_REQUIRED in VerificationPayment() function
 *
 * @since 2012-01-27
 *
 ...
 */
 function displayDetailsPaymentForm($DbConnection, $PaymentID, $FamilyID, $Type, $BillID, $ProcessFormPage, $AccessRules = array())
 {
     ...
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         {
             // Open a form
             openForm("FormDetailsPayment", "post", "$ProcessFormPage?".$GLOBALS["QUERY_STRING"], "",
                      "VerificationPayment('".$GLOBALS["LANG_ERROR_JS_PAYMENT_DATE"]."', '".$GLOBALS["LANG_ERROR_JS_PAYMENT_AMOUNT"]."', '".$GLOBALS["LANG_ERROR_JS_PAYMENT_BANK"]
                                           ."', '".$GLOBALS["LANG_ERROR_JS_PAYMENT_CHECK_NB"]."', '".implode('#', $GLOBALS['CONF_PAYMENTS_MODES_BANK_REQUIRED'])."')");
 ...
 * @author Christophe Javouhey
 * @version 2.4
 *     - 2013-12-18 : v2.0. Add a field to selected a family to generate its monthly bill and a button
 *                    to generate missed bills of some families (because of a technical problem)
 *     - 2020-03-02 : v2.1. Display canteen's prices and nursery's prices
 *     - 2021-10-11 : v2.2. Display monthly contributions for each mode
 *     - 2022-06-06 : v2.3. Taken into account new formats of CONF_NURSERY_PRICES variable
 *     - 2022-07-11 : v2.4. Taken into account new formats of CONF_CANTEEN_PRICES variable
 *
 * @since 2012-02-21
 *
 * @param $DbConnection         DB object             Object of the opened database connection
 ...
 function displayGenerateMonthlyBillsForm($DbConnection, $ProcessFormPage, $Month, $Year, $AccessRules = array(), $bSendMail = NULL)
 {
     ...
     if (isSet($_SESSION["SupportMemberID"]))
     {
         ...
         if (in_array($cUserAccess, array(FCT_ACT_CREATE, FCT_ACT_UPDATE)))
         {
             ...
             // Prices of canteens
             if ((isset($GLOBALS['CONF_CANTEEN_PRICES'][$SchoolYear]))
                 && (!empty($GLOBALS['CONF_CANTEEN_PRICES'][$SchoolYear])))
             {
                 // We detect the canteen mode
                 $ArrayTmpCanteenModes = array_keys($GLOBALS['CONF_CANTEEN_PRICES'][$SchoolYear]);
                 if (isInteger($ArrayTmpCanteenModes[0]))
                 {
                     echo "<ul>\n\t<li>".$GLOBALS['LANG_CANTEEN']." : ".$GLOBALS['CONF_CANTEEN_PRICES'][$SchoolYear][0].$GLOBALS['CONF_PAYMENTS_UNIT']
                          ." / ".$GLOBALS['CONF_CANTEEN_PRICES'][$SchoolYear][1].$GLOBALS['CONF_PAYMENTS_UNIT']."</li>\n</ul>\n";
                 }
                 else
                 {
                     // There are several modes to pay canteen registrations
                     echo "<ul>\n\t<li>".$GLOBALS['LANG_CANTEEN']." : </li>\n</ul>\n";

                     // Display the table header
                     echo "<table cellspacing=\"0\" class=\"MonthlyContributions\">\n<tr><th>".$GLOBALS['LANG_CHILD_GRADE']."</th>";

                     switch(substr($ArrayTmpCanteenModes[0], 0, 2))
                     {
                         case 'G_':
                             // Canteen prices in relation with child grades...
                             $ArrayKeysSecondCanteenParams = array_keys($GLOBALS['CONF_CANTEEN_PRICES'][$SchoolYear][$ArrayTmpCanteenModes[0]]);

                             switch(substr($ArrayKeysSecondCanteenParams[0], 0, 2))
                             {
                                 case 'C_':
                                     // ... and canteen prices in relation with the contribution mode
                                     foreach($GLOBALS['CONF_CANTEEN_PRICES'][$SchoolYear][$ArrayTmpCanteenModes[0]] as $tcc => $SecondCanteenParams)
                                     {
                                         echo "<th>".$GLOBALS['CONF_MONTHLY_CONTRIBUTION_MODES'][substr($tcc, 2)]."</th>";
                                     }

                                     echo "</tr>\n";

                                     foreach($ArrayTmpCanteenModes as $CurrCanteenGrade)
                                     {
                                         echo "<tr><td>".$GLOBALS['CONF_GRADES'][substr($CurrCanteenGrade, 2)]."</td>";

                                         foreach($GLOBALS['CONF_CANTEEN_PRICES'][$SchoolYear][$CurrCanteenGrade] as $tcc => $SecondCanteenParams)
                                         {
                                             echo "<td>".$SecondCanteenParams[0].$GLOBALS['CONF_PAYMENTS_UNIT']." / "
                                                  .$SecondCanteenParams[1].$GLOBALS['CONF_PAYMENTS_UNIT']."</td>";
                                         }
                                 ...
                                 default:
                                     foreach($ArrayTmpCanteenModes as $CurrCanteenGrade)
                                     {
                                         echo "<th>".$GLOBALS['CONF_GRADES'][substr($CurrCanteenGrade, 2)]."</th>";
                                     }
                                     ...
                                     foreach($ArrayTmpCanteenModes as $CurrCanteenGrade)
                                     {
                                         echo "<td>".$GLOBALS['CONF_CANTEEN_PRICES'][$SchoolYear][$CurrCanteenGrade][0].$GLOBALS['CONF_PAYMENTS_UNIT']
                                              ." / ".$GLOBALS['CONF_CANTEEN_PRICES'][$SchoolYear][$CurrCanteenGrade][1].$GLOBALS['CONF_PAYMENTS_UNIT']."</td>";
                                     }
                             ...
                             unset($ArrayKeysSecondCanteenParams);
                             break;
                     }
             ...
             // Prices of nursery
             if ((isset($GLOBALS['CONF_NURSERY_PRICES'][$SchoolYear]))
                 && (!empty($GLOBALS['CONF_NURSERY_PRICES'][$SchoolYear])))
             {
                 // We detect the nursery mode
                 $ArrayTmpNurseryModes = array_keys($GLOBALS['CONF_NURSERY_PRICES'][$SchoolYear]);
                 if (isInteger($ArrayTmpNurseryModes[0]))
                 {
                     // Default mode (AM, PM and other timeslots prices)
                     echo "<ul>\n";
                     echo "\t<li>".$GLOBALS['LANG_NURSERY']." ".$GLOBALS['LANG_AM']." : ".$GLOBALS['CONF_NURSERY_PRICES'][$SchoolYear][0]
                          .$GLOBALS['CONF_PAYMENTS_UNIT']."</li>\n";
                         ...
                         && (!empty($GLOBALS['CONF_NURSERY_PRICES'][$SchoolYear]['OtherTimeslots'])))
                     {
                         ...
                         {
                             echo "\t<li>".$GLOBALS['LANG_NURSERY']." ".$GLOBALS['CONF_NURSERY_OTHER_TIMESLOTS'][$SchoolYear][$ots]['Label']
                                  ." : $CurrentOTSPrice".$GLOBALS['CONF_PAYMENTS_UNIT']."</li>\n";
                         }
                     }

                     echo "\t<li>".$GLOBALS['LANG_NURSERY']." ".$GLOBALS['LANG_PM']." : ".$GLOBALS['CONF_NURSERY_PRICES'][$SchoolYear][1]
                          .$GLOBALS['CONF_PAYMENTS_UNIT']."</li>\n";
                     echo "</ul>\n";
                 }
                 else
                 {
                     echo "<ul>\n\t<li>".$GLOBALS['LANG_NURSERY']." : </li>\n</ul>\n";

                     foreach($ArrayTmpNurseryModes as $tnm => $CurrentNurseryMode)
                     {
                         echo $CurrentNurseryMode.generateBR(1);

                         // Display the table header
                         echo "<table cellspacing=\"0\" class=\"MonthlyContributions\">\n<tr><th>".$GLOBALS['LANG_CHILD_GRADE']."</th>";
                         $ArrayKeysFirstNurseryParams = array_keys($GLOBALS['CONF_NURSERY_PRICES'][$SchoolYear][$CurrentNurseryMode]);
                         switch(substr($ArrayKeysFirstNurseryParams[0], 0, 2))
                         {
                             case 'G_':
                                 // Nursery prices in relation with child grades...
                                 $ArrayKeysSecondNurseryParams = array_keys($GLOBALS['CONF_NURSERY_PRICES'][$SchoolYear][$CurrentNurseryMode][$ArrayKeysFirstNurseryParams[0]]);

                                 switch(substr($ArrayKeysSecondNurseryParams[0], 0, 2))
                                 {
                                     case 'C_':
                                         // ... and nursery prices in relation with the contribution mode
                                         foreach($GLOBALS['CONF_NURSERY_PRICES'][$SchoolYear][$CurrentNurseryMode][$ArrayKeysFirstNurseryParams[0]] as $tnc => $SecondNurseryParams)
                                         {
                                             echo "<th>".$GLOBALS['CONF_MONTHLY_CONTRIBUTION_MODES'][substr($tnc, 2)]."</th>";
                                         }
                                         break;

                                     default:
                                         foreach($ArrayKeysFirstNurseryParams as $CurrNursGrade)
                                         {
                                             echo "<th>".$GLOBALS['CONF_GRADES'][substr($CurrNursGrade, 2)]."</th>";
                                         }
                                         break;
                                 }
                                 break;
                         ...
                         switch(substr($ArrayKeysFirstNurseryParams[0], 0, 2))
                         {
                             case 'G_':
                                 ...
                                 switch(substr($ArrayKeysSecondNurseryParams[0], 0, 2))
                                 {
                                     case 'C_':
                                         // ... and nursery prices in relation with the contribution mode
                                         foreach($GLOBALS['CONF_NURSERY_PRICES'][$SchoolYear][$CurrentNurseryMode] as $cnm => $SecondNurseryParams)
                                         {
                                             echo "<tr><td>".$GLOBALS['CONF_GRADES'][substr($cnm, 2)]."</td>";

                                             foreach($SecondNurseryParams as $snp => $CurrentSecondNurseryParams)
                                             {
                                                 echo "<td>";
                                                 if (isset($CurrentSecondNurseryParams[0]))
                                                 {
                                                     echo $GLOBALS['LANG_AM']." : ".$CurrentSecondNurseryParams[0].$GLOBALS['CONF_PAYMENTS_UNIT']
                                                          .generateBR(1);
                                                 }

                                                 if (isset($CurrentSecondNurseryParams['0_1']))
                                                 {
                                                     // AM and PM
                                                     echo $GLOBALS['LANG_AM']." / ".$GLOBALS['LANG_PM']." : ".$CurrentSecondNurseryParams['0_1']
                                                          .$GLOBALS['CONF_PAYMENTS_UNIT'].generateBR(1);
                                                 }

                                                 if ((isset($GLOBALS['CONF_NURSERY_OTHER_TIMESLOTS'][$SchoolYear]))
                                                     && (!empty($GLOBALS['CONF_NURSERY_OTHER_TIMESLOTS'][$SchoolYear]))
                                                     && (isset($CurrentSecondNurseryParams['OtherTimeslots']))
                                                     && (!empty($CurrentSecondNurseryParams['OtherTimeslots'])))
                                                 {
                                                     // Display prices of other timeslots of the nursery if they exist
                                                     foreach($CurrentSecondNurseryParams['OtherTimeslots'] as $ots => $CurrentOTSPrice)
                                                     {
                                                         echo $GLOBALS['CONF_NURSERY_OTHER_TIMESLOTS'][$SchoolYear][$ots]['Label']
                                                              ." : $CurrentOTSPrice".$GLOBALS['CONF_PAYMENTS_UNIT'].generateBR(1);
                                                     }
                                                 }

                                                 if (isset($CurrentSecondNurseryParams[1]))
                                                 {
                                                     echo $GLOBALS['LANG_PM']." : ".$CurrentSecondNurseryParams[1].$GLOBALS['CONF_PAYMENTS_UNIT']
                                                          .generateBR(1);
                                                 }
                                     ...
                                     default:
                                         echo "<tr><td>&nbsp;</td>";
                                         foreach($GLOBALS['CONF_NURSERY_PRICES'][$SchoolYear][$CurrentNurseryMode] as $cnm => $CurrentNurseryParams)
                                         {
                                             echo "<td>";
                                             if (isset($CurrentNurseryParams[0]))
                                             {
                                                 echo $GLOBALS['LANG_AM']." : ".$CurrentNurseryParams[0].$GLOBALS['CONF_PAYMENTS_UNIT'].generateBR(1);
                                             }

                                             if ((isset($GLOBALS['CONF_NURSERY_OTHER_TIMESLOTS'][$SchoolYear]))
                                                 && (!empty($GLOBALS['CONF_NURSERY_OTHER_TIMESLOTS'][$SchoolYear]))
                                                 && (isset($CurrentNurseryParams['OtherTimeslots']))
                                                 && (!empty($CurrentNurseryParams['OtherTimeslots'])))
                                             {
                                                 // Display prices of other timeslots of the nursery if they exist
                                                 foreach($CurrentNurseryParams['OtherTimeslots'] as $ots => $CurrentOTSPrice)
                                                 {
                                                     echo $GLOBALS['CONF_NURSERY_OTHER_TIMESLOTS'][$SchoolYear][$ots]['Label']
                                                          ." : $CurrentOTSPrice".$GLOBALS['CONF_PAYMENTS_UNIT'].generateBR(1);
                                                 }
                                             ...
                                             if (isset($CurrentNurseryParams[1]))
                                             {
                                                 echo $GLOBALS['LANG_PM']." : ".$CurrentNurseryParams[1].$GLOBALS['CONF_PAYMENTS_UNIT'].generateBR(1);
                                             }
             ...
             // Display monthly contributions for each mode and each nb of children
             if (isset($GLOBALS['CONF_CONTRIBUTIONS_MONTHLY_AMOUNTS'][$SchoolYear]))
             {
                 // We get the max number of prices for children
                 $iNbMaxChildren = count($GLOBALS['CONF_CONTRIBUTIONS_MONTHLY_AMOUNTS'][$SchoolYear][MC_DEFAULT_MODE]);

                 // Display the table header
                 echo "<table cellspacing=\"0\" class=\"MonthlyContributions\">\n<tr><th>".$GLOBALS['LANG_NB_CHILDREN']."</th>";
                 for($i = 1; $i <= $iNbMaxChildren; $i++)
                 {
                     echo "<th>$i</th>";
                 }
                 echo "</tr>\n";
                 ...
                 foreach($GLOBALS['CONF_CONTRIBUTIONS_MONTHLY_AMOUNTS'][$SchoolYear] as $m => $CurrentMode)
                 {
                     echo "<tr><td>".$GLOBALS['CONF_MONTHLY_CONTRIBUTION_MODES'][$m]."</td>";
                     foreach($CurrentMode as $c => $ChildPrice)
                     {
                         echo "<td>$ChildPrice ".$GLOBALS['CONF_PAYMENTS_UNIT']."</td>";
                     }
                     echo "</tr>\n";
                 }
...
/**
 * Display the details of a bill, in the current web page, in the graphic interface in XHTML
 *
 * @author Christophe Javouhey
 * @version 2.0
 *     - 2022-06-07 : v2.0. Allow to edit the bill. Taken into account $Mode parameter
 *
 * @since 2012-03-16
 *
 * @param $DbConnection             DB object             Object of the opened database connection
 ...
 * @param $ProcessFormPage          String                URL of the page which will process the form
 * @param $AccessRules              Array of Integers     List used to select only some support members
 *                                                        allowed to create or update bills
 * @param $Mode                     String                To allow or not to edit the bill ("view" or "edit" modes)
 */
 function displayDetailsBillForm($DbConnection, $BillID, $ProcessFormPage, $AccessRules = array(), $Mode = 'view')
 {
     // The supporter must be logged,
     if (isset($_SESSION["SupportMemberID"]))
     {
         if ((empty($Mode)) || (!in_array($Mode, array('view', 'edit'))))
         {
             $Mode = 'view';
         }
         ...
         if ((!empty($BillID)) && ($BillID > 0))
         {
             ...
             {
                 case 'edit':
                     // Display the bill to be edited by allowed support members
                     if (in_array($cUserAccess, array(FCT_ACT_CREATE, FCT_ACT_UPDATE)))
                     {
                         // Open a form
                         openForm("FormDetailsBill", "post", "$ProcessFormPage?".$GLOBALS["QUERY_STRING"], "", "VerificationBill()");

                         // <<< Bill ID >>>
                         if (isExistingBill($DbConnection, $BillID))
                         {
                             // We get the details of the bill
                             $BillRecord = getTableRecordInfos($DbConnection, "Bills", $BillID);

                             // Get infos about the family
                             $FamilyRecord = getTableRecordInfos($DbConnection, "Families", $BillRecord['FamilyID']);
                         }
                         else
                         {
                             // Error, the bank doesn't exist
                         ...
                         // Display the table (frame) where the form will take place
                         openStyledFrame($GLOBALS["LANG_BILL"]." ($BillID)", "Frame", "Frame", "DetailsNews");

                         $BillFamilyLastname = $FamilyRecord['FamilyLastname'];
                         $BillForDate = date($GLOBALS['CONF_DATE_DISPLAY_FORMAT'], strtotime($BillRecord['BillForDate']));
                         $BillPreviousBalance = $BillRecord['BillPreviousBalance'].' '.$GLOBALS['CONF_PAYMENTS_UNIT'];

                         // <<< Monthly contribution INPUTFIELD >>>
                         $BillMonthlyContribution = generateInputField("fBillMonthlyContribution", "text", "10", "10",
                                                                       $GLOBALS["LANG_BILL_MONTHLY_CONTRIBUTION"]." (".$GLOBALS["LANG_FAMILY_BILL_AMOUNT"].").",
                                                                       $BillRecord['BillMonthlyContribution']);

                         // <<< Nb canteen registrations INPUTFIELD >>>
                         $BillNbCanteenRegistrations = generateInputField("sBillNbCanteenRegistrations", "text", "5", "5", $GLOBALS["LANG_CANTEEN"],
                                                                          $BillRecord['BillNbCanteenRegistrations']);

                         // <<< Canteen amount INPUTFIELD >>>
                         $BillCanteenAmount = generateInputField("fBillCanteenAmount", "text", "10", "10", $GLOBALS["LANG_CANTEEN"]." (".$GLOBALS["LANG_FAMILY_BILL_AMOUNT"].").",
                                                                 $BillRecord['BillCanteenAmount']);

                         // <<< Nb nursery registrations INPUTFIELD >>>
                         $BillNbNurseryRegistrations = generateInputField("sBillNbNurseryRegistrations", "text", "5", "5", $GLOBALS["LANG_NURSERY"],
                                                                          $BillRecord['BillNbNurseryRegistrations']);

                         // <<< Nursery amount INPUTFIELD >>>
                         $BillNurseryAmount = generateInputField("fBillNurseryAmount", "text", "10", "10", $GLOBALS["LANG_NURSERY"]." (".$GLOBALS["LANG_FAMILY_BILL_AMOUNT"].").",
                                                                 $BillRecord['BillNurseryAmount']);

                         // <<< BillWithoutMealAmount INPUTFIELD >>>
                         $BillWithoutMealAmount = generateInputField("fBillWithoutMealAmount", "text", "10", "10",
                                                                     $GLOBALS["LANG_BILL_CANTEEN_WITHOUT_MEAL"]." (".$GLOBALS["LANG_FAMILY_BILL_AMOUNT"].").",
                                                                     $BillRecord['BillWithoutMealAmount']);

                         // <<< BillOtherAmount INPUTFIELD >>>
                         $BillOtherAmount = generateInputField("fBillOtherAmount", "text", "10", "10",
                                                               $GLOBALS["LANG_BILL_OTHER_AMOUNT"]." (".$GLOBALS["LANG_FAMILY_BILL_AMOUNT"].").",
                                                               $BillRecord['BillOtherAmount']);

                         // Sub-total and total
                         $BillSubTotal = getBillAmount($DbConnection, $BillID, '');
                         $BillTotalToPay = getBillAmount($DbConnection, $BillID, WITHOUT_PREVIOUS_BALANCE);

                         // Display the form
                         echo "<table cellspacing=\"0\" cellpadding=\"0\">\n<tr>\n\t<td class=\"Label\">".$GLOBALS["LANG_FAMILY"]."</td><td class=\"Value\">$BillFamilyLastname</td><td class=\"Label\">".$GLOBALS["LANG_BILL_FOR_DATE"]."</td><td class=\"Value\">$BillForDate</td>\n</tr>\n";
                         echo "<tr>\n\t<td class=\"Label\">".$GLOBALS["LANG_BILL_PREVIOUS_MONTHS_BALANCE"]."</td><td class=\"Value\">$BillPreviousBalance</td><td class=\"Label\">".$GLOBALS["LANG_BILL_MONTHLY_CONTRIBUTION"]."</td><td class=\"Value\">$BillMonthlyContribution</td>\n</tr>\n";
                         echo "<tr>\n\t<td class=\"Label\">".$GLOBALS["LANG_CANTEEN"]."</td><td class=\"Value\">$BillNbCanteenRegistrations</td><td class=\"Label\">".$GLOBALS["LANG_FAMILY_BILL_AMOUNT"]."</td><td class=\"Value\">$BillCanteenAmount</td>\n</tr>\n";
                         echo "<tr>\n\t<td class=\"Label\">".$GLOBALS["LANG_NURSERY"]."</td><td class=\"Value\">$BillNbNurseryRegistrations</td><td class=\"Label\">".$GLOBALS["LANG_FAMILY_BILL_AMOUNT"]."</td><td class=\"Value\">$BillNurseryAmount</td>\n</tr>\n";
                         echo "<tr>\n\t<td class=\"Label\">".$GLOBALS["LANG_BILL_CANTEEN_WITHOUT_MEAL"]."</td><td class=\"Value\">$BillWithoutMealAmount</td><td class=\"Label\">".$GLOBALS["LANG_BILL_OTHER_AMOUNT"]."</td><td class=\"Value\">$BillOtherAmount</td>\n</tr>\n";
                         echo "<tr>\n\t<td class=\"Label\">".$GLOBALS["LANG_BILL_SUB_TOTAL"]."</td><td class=\"Value\">$BillSubTotal</td><td class=\"Label\">".$GLOBALS["LANG_BILL_TOTAL_TO_PAY"]."</td><td class=\"Value\">$BillTotalToPay</td>\n</tr>\n";
                         echo "</table>\n";
                 ...
                 default:
                 case 'view':
                     // Display the bill only to be viewed
                     if (in_array($cUserAccess, array(FCT_ACT_CREATE, FCT_ACT_UPDATE, FCT_ACT_READ_ONLY)))
                     {
         ...
         else
         {
             // The supporter isn't allowed to view the bill
             openParagraph('ErrorMsg');
             echo $GLOBALS["LANG_ERROR_WRONG_BILL_ID"];
             closeParagraph();
         }
     }
     else
