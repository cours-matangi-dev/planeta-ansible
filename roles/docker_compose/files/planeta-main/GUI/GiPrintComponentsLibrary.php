 ...
 * @author Christophe Javouhey
 * @version 2.2
 *     - 2014-06-03 : v2.0. Use params to send translation of messages to the XSL template
 *     - 2021-10-11 : v2.1. Taken into account "infos" message about the bill
 *     - 2022-06-07 : v2.2. Taken into account BillOtherAmount field
 *
 * @since 2012-02-23
 *
 * @param $DbConnection         DB object             Object of the opened database connection
 ...
 * @author Christophe Javouhey
 * @version 2.1
 *     - 2014-06-05 : v1.1. Change the margins of the annual bill
 *     - 2016-06-08 : v2.0. Taken into account tax receipt of donation
 *     - 2021-10-11 : v2.1. Taken into account "infos" message about the bill
 *
 * @since 2012-05-04
 *
     ...
     if ((!empty($SourceHtml)) && (!empty($DestPdf)) && (file_exists($SourceHtml)))
     {
         ...
         switch($DocumentType)
         {
             ...
             case ALL_MONTHLY_BILLS_DOCTYPE:
                 ...
                 // Infos about the bill
                 if (empty($GLOBALS['LANG_BILL_INFOS']))
                 {
                     // No infos message about the bill
                     $BillHeight = 8.5;
                 }
                 else
                 {
                     // There is a info message about the bill : we compute the number of lines to display it
                     $pdf->SetFont("Arial", "", 7);
                     $sWidth = $pdf->GetStringWidth($GLOBALS['LANG_BILL_INFOS']);
                     $iInfosNbLines = ceil($sWidth / ($BillWidth - (2 * $MarginBorder)));
                     $BillHeight = 8.5 + $iInfosNbLines * 0.25;
                 }

                 $pdf->SetMargins(0.5, $MarginTop, 0.5);
                 $pdf->SetFont("Arial", "", 11);
                 $pdf->SetTextColor(0, 0, 0);
                 ...
                 for($b = 0; $b < $iListBillsSize; $b++)
                 {
                     ...
                     switch($iMod)
                     {
                         ...
                         case 3:
                             fpdfBill($pdf, $XmlDoc->listbills->bill[$b], NULL, $BillHeight + $MarginTop);
                             break;

                         case 0:
                             fpdfBill($pdf, $XmlDoc->listbills->bill[$b], 14.5, $BillHeight + $MarginTop);
                             break;
                     }
                 }
                 break;
 ...
 * @author Christophe Javouhey
 * @version 1.4
 *     - 2014-02-03 : v1.1. Taken into account BillNurseryNbDelays field
 *     - 2014-06-03 : v1.2. Use LANG consts to translate all messages of the bill
 *     - 2021-10-11 : v1.3. Taken into account "infos" message about the bill and patch the bug about charset problem (UTF-8)
 *     - 2022-06-07 : v1.4. Taken into account BillOtherAmount field
 *
 * @since 2012-05-04
 *
 ...
 */
 function fpdfBill(&$pdf, $XmlData, $PosX = NULL, $PosY = NULL)
 {
     if (!empty($XmlData))
     {
         ...
         // Patch the charset problem of the bill (UTF-8 displayed)
         if ((strToUpper($GLOBALS['CONF_CHARSET']) != 'UTF-8')
             && (mb_detect_encoding($XmlData->detailsbill[0]->familylastname) == 'UTF-8'))
         {
             $XmlData->detailsbill[0]->familylastname = utf8_decode($XmlData->detailsbill[0]->familylastname);
         }
         ...
         // We take into account other amount if <> 0.00
         if ((!empty($XmlData->detailsbill[0]->billotheramount)) && ($XmlData->detailsbill[0]->billotheramount <> 0.00))
         {
             $pdf->SetFont("Arial", "", 14);
             $pdf->Text($x + $MarginBorder, $y + 4.55, $GLOBALS['LANG_BILL_OTHER_AMOUNT']);
             $pdf->Text($x + $MarginBorder + $sMaxLabelWidth + 0.2, $y + 4.55, $XmlData->detailsbill[0]->billmonthyear);
             $pdf->SetFont("Arial", "", 12);
             $sWidth = $pdf->GetStringWidth($XmlData->detailsbill[0]->billotheramount);
             $pdf->Text($x + $BillWidth - $sWidth - $MarginBorder, $y + 4.55, $XmlData->detailsbill[0]->billotheramount);
         }
         ...
         {
             $pdf->SetFont("Arial", "", 7);
             $sSpaceWidth = $pdf->GetStringWidth(' ');
             $sWidth = $pdf->GetStringWidth($GLOBALS['LANG_BILL_INFOS']);
             $ArrayWords = explode(' ', $GLOBALS['LANG_BILL_INFOS']);
             $iCurentWidth = 0;
             $iCurrentLine = 1;
             $iMaxWidth = $BillWidth - (2 * $MarginBorder);

             foreach($ArrayWords as $w => $CurrentWord)
             {
                 $sWidth = $pdf->GetStringWidth($CurrentWord);
                 if ($iCurentWidth > 0)
                 {
                     $iCurentWidth += $sSpaceWidth;
                 }

                 if ($iCurentWidth + $sWidth > $iMaxWidth)
                 {
                     // New line
                     $iCurentWidth = 0;
                     $iCurrentLine++;
                 ...
                 $pdf->Text($x + $MarginBorder + $iCurentWidth, $y + 8.3 + ($iCurrentLine * 0.25), $CurrentWord);
                 $iCurentWidth += $sWidth;
             }
         }
     }
