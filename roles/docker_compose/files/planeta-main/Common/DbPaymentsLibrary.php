 ...
 * @version 2.4
 *     - 2013-02-05 : taken into account the new field "PaymentReceiptDate"
 *     - 2013-11-22 : v2.0. Allow to take into account more fields in the SELECT and teh array result
 *                    in relation with
 *     - 2013-12-16 : v2.1. Taken into account the "PaymentUsedAmount" field
 *     - 2015-06-03 : v2.2. Allow to filter payments on the PaymentDate field
 *     - 2018-01-22 : v2.3. Allow to filter payements not linked to bills
 *     - 2021-11-25 : v2.4. Allow to filter payments on the PaymentReceiptDate and PaymentMode fields
 *
 * @since 2012-01-26
 *
 * @param $DbConnection              DB object      Object of the opened database connection
 ...
 function getFamilyPayments($DbConnection, $FamilyID, $ArrayParams = array(), $OrderBy = 'PaymentDate')
 {
     if ($FamilyID > 0)
     {
         ...
         if (!empty($ArrayParams))
         {
             ...
             if ((isset($ArrayParams['PaymentMode'])) && (!empty($ArrayParams['PaymentMode'])))
             {
                 $Conditions .= " AND p.PaymentMode IN ".constructSQLINString($ArrayParams["PaymentMode"]);
             }
             ...
             if ((isset($ArrayParams['PaymentDate'])) && (count($ArrayParams['PaymentDate']) >= 2))
             {
                 ...
                 if (count($ArrayParams['PaymentDate']) == 4)
                 {
                     // $ArrayParams['PaymentDate'][2] contains the operator (>, >=, =...) and
                     // $ArrayParams['PaymentDate'][3] contains the date in english format (YYYY-MM-DD or YYYY-MM-DD HH:MM:SS)
                     $Conditions .= " AND p.PaymentDate ".$ArrayParams['PaymentDate'][2]." \"".$ArrayParams['PaymentDate'][3]."\"";
                 }
             }

             if ((isset($ArrayParams['PaymentReceiptDate'])) && (count($ArrayParams['PaymentReceiptDate']) >= 2))
             {
                 // $ArrayParams['PaymentReceiptDate'][0] contains the operator (>, >=, =...) and
                 // $ArrayParams['PaymentReceiptDate'][1] contains the date in english format (YYYY-MM-DD or YYYY-MM-DD HH:MM:SS)
                 $Conditions .= " AND p.PaymentReceiptDate ".$ArrayParams['PaymentReceiptDate'][0]." \"".$ArrayParams['PaymentReceiptDate'][1]."\"";

                 if (count($ArrayParams['PaymentReceiptDate']) == 4)
                 {
                     // $ArrayParams['PaymentReceiptDate'][2] contains the operator (>, >=, =...) and
                     // $ArrayParams['PaymentReceiptDate'][3] contains the date in english format (YYYY-MM-DD or YYYY-MM-DD HH:MM:SS)
                     $Conditions .= " AND p.PaymentReceiptDate ".$ArrayParams['PaymentReceiptDate'][2]." \"".$ArrayParams['PaymentReceiptDate'][3]."\"";
                 }
             }
         }
 ...
 * @author Christophe Javouhey
 * @version 1.4
 *     - 2013-02-05 : v1.1. Taken into account the new field "PaymentReceiptDate"
 *     - 2013-11-21 : v1.2. Taken into account the new field "PaymentBillPartAmount"
 *     - 2013-12-16 : v1.3. Taken into account the new fields "PaymentUsedAmount" and "BillPaidAmount"
 ...
 function getBillsOfPayment($DbConnection, $PaymentID, $ArrayParams = array(), $OrderBy = 'BillID')
 {
     if ((is_null($PaymentID)) || ((!empty($PaymentID)) && ($PaymentID > 0)))
     {
         ...
         $DbResult = $DbConnection->query("SELECT b.BillID, b.BillForDate, b.BillDeposit, b.BillPreviousBalance,
                                          ...
                                          b.BillOtherAmount, b.BillPaidAmount, b.BillPaid, p.PaymentID, p.PaymentDate, p.PaymentReceiptDate,
                                          p.PaymentType, p.PaymentAmount, p.PaymentUsedAmount, pb.PaymentBillPartAmount
         ...
         if (!DB::isError($DbResult))
         {
             ...
             while($Record = $DbResult->fetchRow(DB_FETCHMODE_ASSOC))
             {
                 ...
                 $ArrayRecords["BillOtherAmount"][] = $Record["BillOtherAmount"];
