...
/**
 * Treat laundry registrations data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-06
 ...
 */
 function dbLaundryRegistrationsGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         ...
         // First, we delete the selected laundry registrations too old
         $sDateCondition = " LaundryRegistrationDate < ".date('Y-m-d', strtotime("5 years ago"));
         $sLogDateCondition = " AND LogEventDate < ".date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("LaundryRegistrationDate", $ArrayParams)) && (count($ArrayParams["LaundryRegistrationDate"]) == 2))
         {
             $sDateCondition = " LaundryRegistrationDate ".$ArrayParams["LaundryRegistrationDate"][0]
                               ." \"".$ArrayParams["LaundryRegistrationDate"][1]."\"";

             $sLogDateCondition = " AND LogEventDate ".$ArrayParams["LaundryRegistrationDate"][0]
                                  ." \"".$ArrayParams["LaundryRegistrationDate"][1]."\"";
         }

         $DbResult = $DbConnection->query("DELETE FROM LaundryRegistrations WHERE $sDateCondition");
         if (!DB::isError($DbResult))
         {
             // Laundry registrations deleted
             $bRegistrationsDeleted = TRUE;
         }

         // Then, we delete event logs
         $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_LAUNDRY."\"
                                           $sLogDateCondition");
         if (!DB::isError($DbResult))
         {
             // Event logs about laundry registrations deleted
             $bLogsDeleted = TRUE;
         }
