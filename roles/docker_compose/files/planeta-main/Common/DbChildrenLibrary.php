...
/**
 * Treat children data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.1
 *     - 2021-12-29 : v1.1. Add a " " in the SQL query about anonymization of the firstname and set default meal
 *
 * @since 2021-05-06
 *
 * @param $DbConnection                 DB object      Object of the opened database connection
 ...
 */
 function dbChildrenGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         $ArrayConcernedChildrenID = array();
         $bChildrenAnonymized = FALSE;
         $bLogsDeleted = FALSE;
         ...
         $DbResult = $DbConnection->query("SELECT c.ChildID
                                           FROM Children c, Families f
                                           WHERE c.FamilyID = f.FamilyID AND (c.ChildEmail IS NULL OR c.ChildEmail = '')
                                           AND c.ChildFirstname REGEXP 'Ch[0-9]+' = 0 $sDateCondition
                                           ORDER BY c.ChildID");
         ...
         if (!DB::isError($DbResult))
         {
             if ($DbResult->numRows() >= 0)
             {
                 while($Record = $DbResult->fetchRow(DB_FETCHMODE_ASSOC))
                 {
                     $ArrayConcernedChildrenID[] = $Record['ChildID'];
                 }
             }
         }

         if (empty($ArrayConcernedChildrenID))
         {
             $bChildrenAnonymized = TRUE;
             $bLogsDeleted = TRUE;
         }
         else
         {
             // Anonymization of the firstname and set default meal
             $DbResult = $DbConnection->query("UPDATE Children SET ChildFirstname = CONCAT('Ch', ChildID),
                                               ChildWithoutPork = ".CANTEEN_REGISTRATION_DEFAULT_MEAL
                                              ." WHERE ChildID IN ".constructSQLINString($ArrayConcernedChildrenID));
             if (!DB::isError($DbResult))
             {
                 // Children anonymized
                 $bChildrenAnonymized = TRUE;
             ...
             $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_FAMILY."\"
                                               AND LogEventService = \"".EVT_SERV_CHILD."\"
                                               AND LogEventItemID IN ".constructSQLINString($ArrayConcernedChildrenID));
             if (!DB::isError($DbResult))
             {
                 // Event logs about children deleted
                 $bLogsDeleted = TRUE;
             }
         }

         return (($bChildrenAnonymized) && ($bLogsDeleted));
     }
...
/**
 * Treat children levels history data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-06
 ...
 * @return Boolean                      TRUE if GDPR treatment is done, FALSE otherwise
 */
 function dbHistoLevelsChildrenGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         $ArrayConcernedHistoryID = array();
         $bHistoryDeleted = FALSE;

         // First, we get concerned children levels history
         $sDateCondition = " AND f.FamilyDesactivationDate IS NOT NULL AND f.FamilyDesactivationDate < "
                           .date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("FamilyDesactivationDate", $ArrayParams)) && (count($ArrayParams["FamilyDesactivationDate"]) == 2))
         {
         ...
         $DbResult = $DbConnection->query("SELECT h.HistoLevelChildID
                                           FROM HistoLevelsChildren h, Children c, Families f
                                           WHERE h.ChildID = c.ChildID AND c.FamilyID = f.FamilyID
                                           AND (c.ChildEmail IS NULL OR c.ChildEmail = '') $sDateCondition
                                           ORDER BY h.HistoLevelChildID");
         ...
         if (!DB::isError($DbResult))
         {
             if ($DbResult->numRows() >= 0)
             {
                 while($Record = $DbResult->fetchRow(DB_FETCHMODE_ASSOC))
                 {
                     $ArrayConcernedHistoryID[] = $Record['HistoLevelChildID'];
                 }
             }
         }

         if (empty($ArrayConcernedHistoryID))
         {
             $bHistoryDeleted = TRUE;
         }
         else
         {
             // We delete selected children levels history
             $DbResult = $DbConnection->query("DELETE FROM HistoLevelsChildren
                                               WHERE HistoLevelChildID IN ".constructSQLINString($ArrayConcernedHistoryID));
             if (!DB::isError($DbResult))
             {
                 // History deleted
                 $bHistoryDeleted = TRUE;
...
/**
 * Treat suspensions of children data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-06
 ...
 * @return Boolean                      TRUE if GDPR treatment is done, FALSE otherwise
 */
 function dbSuspensionsGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         $ArrayConcernedSuspensionID = array();
         $bSuspensionsDeleted = FALSE;
         $bLogsDeleted = FALSE;

         // First, we get concerned suspensions
         $sDateCondition = " AND f.FamilyDesactivationDate IS NOT NULL AND f.FamilyDesactivationDate < "
                           .date('Y-m-d', strtotime("5 years ago"));
         $sLogDateCondition = " AND LogEventDate < ".date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("FamilyDesactivationDate", $ArrayParams)) && (count($ArrayParams["FamilyDesactivationDate"]) == 2))
         ...
         if (!DB::isError($DbResult))
         {
             if ($DbResult->numRows() >= 0)
             {
                 while($Record = $DbResult->fetchRow(DB_FETCHMODE_ASSOC))
                 {
                     $ArrayConcernedSuspensionID[] = $Record['SuspensionID'];
                 }
             }
         }

         if (empty($ArrayConcernedSuspensionID))
         {
             $bSuspensionsDeleted = TRUE;
             $bLogsDeleted = TRUE;
         }
         else
         {
             // We delete selected suspensions
             $DbResult = $DbConnection->query("DELETE FROM Suspensions
                                               WHERE SuspensionID IN ".constructSQLINString($ArrayConcernedSuspensionID));
             if (!DB::isError($DbResult))
             {
                 // Suspensions deleted
                 $bSuspensionsDeleted = TRUE;
             ...
             // Then, we delete event logs
             $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_FAMILY."\"
                                               AND LogEventService = \"".EVT_SERV_SUSPENSION."\"
                                               AND LogEventItemID IN ".constructSQLINString($ArrayConcernedSuspensionID));
             if (!DB::isError($DbResult))
             {
                 // Event logs about suspensions deleted
                 $bLogsDeleted = TRUE;
             }
         }

         return (($bSuspensionsDeleted) && ($bLogsDeleted));
     }
