 ...
 *     - 2020-01-08 : v3.1. Taken into account DKIM parameters in $CONF_EMAIL_SMTP_SERVERS
 *
 * @since 2004-06-12
 *
 ...
 */
 function sendEmail($Session, $MailingList, $Subject, $TemplateName, $ReplaceInTemplate = array(), $Attachment = array(), $SpecialTemplatePath = '', $SpecialReplyTo = '', $ContentType = 'text/html')
 {
     ...
     if ($CurrentSMTPServer != 'localhost')
     {
         ...
         {
             // Authentification to use the SMTP server
         ...
         if (isset($GLOBALS['CONF_EMAIL_SMTP_SERVERS'][$CurrentSMTPServer]['DKIM']))
         {
             // DKIM authentification
             $Mailer->DKIM_selector = $GLOBALS['CONF_EMAIL_SMTP_SERVERS'][$CurrentSMTPServer]['DKIM']['DKIMSelector'];
             $Mailer->DKIM_domain = $GLOBALS['CONF_EMAIL_SMTP_SERVERS'][$CurrentSMTPServer]['DKIM']['DKIMDomain'];

             if ((isset($GLOBALS['CONF_EMAIL_SMTP_SERVERS'][$CurrentSMTPServer]['DKIM']['DKIMIdentity']))
                 && (!empty($GLOBALS['CONF_EMAIL_SMTP_SERVERS'][$CurrentSMTPServer]['DKIM']['DKIMIdentity'])))
             {
                 // E-mail address used to send the e-mail as a rule
                 $Mailer->DKIM_identity = $GLOBALS['CONF_EMAIL_SMTP_SERVERS'][$CurrentSMTPServer]['DKIM']['DKIMIdentity'];
             }

             $bDKIMKeySet = FALSE;
             if ((isset($GLOBALS['CONF_EMAIL_SMTP_SERVERS'][$CurrentSMTPServer]['DKIM']['DKIMPrivateFileKey']))
                 && (!empty($GLOBALS['CONF_EMAIL_SMTP_SERVERS'][$CurrentSMTPServer]['DKIM']['DKIMPrivateFileKey'])))
             {
                 // DKIM key is in a file
                 $Mailer->DKIM_private = $GLOBALS['CONF_EMAIL_SMTP_SERVERS'][$CurrentSMTPServer]['DKIM']['DKIMPrivateFileKey'];
                 $bDKIMKeySet = TRUE;
             }

             if ((!$bDKIMKeySet) && (isset($GLOBALS['CONF_EMAIL_SMTP_SERVERS'][$CurrentSMTPServer]['DKIM']['DKIMPrivateStringKey']))
                 && (!empty($GLOBALS['CONF_EMAIL_SMTP_SERVERS'][$CurrentSMTPServer]['DKIM']['DKIMPrivateStringKey'])))
             {
                 // DKIM key is in a string
                 $Mailer->DKIM_private_string = $GLOBALS['CONF_EMAIL_SMTP_SERVERS'][$CurrentSMTPServer]['DKIM']['DKIMPrivateStringKey'];
                 $bDKIMKeySet = TRUE;
             }
         }
     }
