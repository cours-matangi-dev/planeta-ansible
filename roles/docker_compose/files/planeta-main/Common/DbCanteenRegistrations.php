 ...
 *     - 2014-02-06 : v1.3. Patch pb about search modes on "CanteenRegistrationForDate" field
 *     - 2022-07-13 : v1.4. Allow to filter children by some parameters
 *
 * @since 2012-01-28
 *
 * @param $DbConnection         DB object            Object of the opened database connection
 ...
 */
 function getChildrenListForCanteenPlanning($DbConnection, $StartDate, $EndDate = NULL, $OrderBy = 'ChildClass', $bActivated = FALSE, $bValided = FALSE, $Mode = PLANNING_BETWEEN_DATES, $ArrayParams = array())
 {
     if (empty($EndDate))
     {
     ...
     if (!empty($ArrayParams))
     {
         ...
         {
             $Conditions1 .= " AND f.FamilyID IN ".constructSQLINString($ArrayParams['FamilyID']);
         }
     }
     ...
     if (!empty($ArrayParams))
     {
         if ((isset($ArrayParams['ChildClass'])) && (count($ArrayParams['ChildClass']) > 0))
         {
             ...
             $Having .= "ChildClass IN ".constructSQLINString($ArrayParams['ChildClass']);
         }
         ...
         if ((isset($ArrayParams['ChildGrade'])) && (count($ArrayParams['ChildGrade']) > 0))
         {
             ...
             $Having .= "ChildGrade IN ".constructSQLINString($ArrayParams['ChildGrade']);
         }

         if ((isset($ArrayParams['ChildWithoutPork'])) && (count($ArrayParams['ChildWithoutPork']) > 0))
         {
             if (!empty($Having))
             {
                 $Having .= " AND ";
             ...
             $Having .= "ChildWithoutPork IN ".constructSQLINString($ArrayParams['ChildWithoutPork']);
         }
     }
     ...
     $DbResult = $DbConnection->query("$Select $From $Where GROUP BY ChildID $Having ORDER BY $OrderBy");
...
/**
 * Treat canteen registrations data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-05
 ...
 */
 function dbCanteenRegistrationsGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         $bStatsDone = FALSE;
         $bRegistrationsDeleted = FALSE;
         $bLogsDeleted = FALSE;

         // First, we store in the Stats table the too old canteen registrations
         $sDateCondition = " CanteenRegistrationForDate < ".date('Y-m-d', strtotime("5 years ago"));
         $sLogDateCondition = " AND LogEventDate < ".date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("CanteenRegistrationForDate", $ArrayParams)) && (count($ArrayParams["CanteenRegistrationForDate"]) == 2))
         {
             $sDateCondition = " CanteenRegistrationForDate ".$ArrayParams["CanteenRegistrationForDate"][0]
                               ." \"".$ArrayParams["CanteenRegistrationForDate"][1]."\"";

             $sLogDateCondition = " AND LogEventDate ".$ArrayParams["CanteenRegistrationForDate"][0]
                                  ." \"".$ArrayParams["CanteenRegistrationForDate"][1]."\"";
         }

         $DbResult = $DbConnection->query("SELECT DATE_FORMAT(CanteenRegistrationForDate, '%Y-%m') AS StatPeriod,
                                           '".STAT_TYPE_NB_CANTEEN_REGISTRATIONS."' AS StatType,
                                           CONCAT(CanteenRegistrationChildGrade, '#', CanteenRegistrationWithoutPork) AS StatSubType,
                                           COUNT(CanteenRegistrationID) AS StatValue
                                           FROM CanteenRegistrations
                                           WHERE $sDateCondition
         ...
         // Now, we delete the selected canteen registrations too old
         $DbResult = $DbConnection->query("DELETE FROM CanteenRegistrations WHERE $sDateCondition");
         if (!DB::isError($DbResult))
         {
             // Canteen registrations deleted
             $bRegistrationsDeleted = TRUE;
         }

         // Then, we delete event logs
         $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_CANTEEN."\"
                                          AND LogEventService = \"".EVT_SERV_PLANNING."\" $sLogDateCondition");
         if (!DB::isError($DbResult))
         {
             // Event logs about canteen registrations deleted
             $bLogsDeleted = TRUE;
         }

         return (($bStatsDone) && ($bRegistrationsDeleted) && ($bLogsDeleted));
     }
...
/**
 * Treat more meals registrations data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-05
 ...
 * @return Boolean                      TRUE if GDPR treatment is done, FALSE otherwise
 */
 function dbMoreMealsGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         $bStatsDone = FALSE;
         ...
         // First, we store in the Stats table the too old more meals
         $sDateCondition = " MoreMealForDate < ".date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("MoreMealForDate", $ArrayParams)) && (count($ArrayParams["MoreMealForDate"]) == 2))
         {
             $sDateCondition = " MoreMealForDate ".$ArrayParams["MoreMealForDate"][0]
                               ." \"".$ArrayParams["MoreMealForDate"][1]."\"";
         }

         $DbResult = $DbConnection->query("SELECT DATE_FORMAT(MoreMealForDate, '%Y-%m') AS StatPeriod,
                                           '".STAT_TYPE_NB_MORE_MEALS."' AS StatType, 'WithPork' AS StatSubType,
                                           SUM(MoreMealQuantity) AS StatValue
                                           FROM MoreMeals
                                           WHERE $sDateCondition
                                           GROUP BY StatPeriod, StatType, StatSubType
                                           UNION
                                           SELECT DATE_FORMAT(MoreMealForDate, '%Y-%m') AS StatPeriod,
                                           '".STAT_TYPE_NB_MORE_MEALS."' AS StatType, 'WithoutPork' AS StatSubType,
                                           SUM(MoreMealWithoutPorkQuantity) AS StatValue
                                           FROM MoreMeals
                                           WHERE $sDateCondition
                                           GROUP BY StatPeriod, StatType, StatSubType
                                           ORDER BY StatPeriod");
         ...
         // Now, we delete the selected more meals registrations too old
         $DbResult = $DbConnection->query("DELETE FROM MoreMeals WHERE $sDateCondition");
         if (!DB::isError($DbResult))
         {
             // More meals deleted
             $bRegistrationsDeleted = TRUE;
         ...
         return (($bStatsDone) && ($bRegistrationsDeleted));
     }
