...
/**
 * Treat meeting rooms regsitrations data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-06
 ...
 */
 function dbMeetingRoomsRegistrationsGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         $bRegistrationsAnonymized = FALSE;
         $bLogsDeleted = FALSE;

         // First, we anonymized the selected meeting rooms registrations too old
         $sDateCondition = " MeetingRoomRegistrationStartDate < ".date('Y-m-d', strtotime("5 years ago"));
         $sLogDateCondition = " AND LogEventDate < ".date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("MeetingRoomRegistrationStartDate", $ArrayParams)) && (count($ArrayParams["MeetingRoomRegistrationStartDate"]) == 2))
         {
             $sDateCondition = " MeetingRoomRegistrationStartDate ".$ArrayParams["MeetingRoomRegistrationStartDate"][0]
                               ." \"".$ArrayParams["MeetingRoomRegistrationStartDate"][1]."\"";

             $sLogDateCondition = " AND LogEventDate ".$ArrayParams["MeetingRoomRegistrationStartDate"][0]
                                  ." \"".$ArrayParams["MeetingRoomRegistrationStartDate"][1]."\"";
         }
         ...
         $DbResult = $DbConnection->query("UPDATE MeetingRoomsRegistrations SET SupportMemberID = $AnonymizedSupportMemberID
                                           WHERE $sDateCondition $sSupporterCondition");
         if (!DB::isError($DbResult))
         {
             // Meeting rooms registrations anonymized
             $bRegistrationsAnonymized = TRUE;
         }

         // Then, we delete event logs
         $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_MEETING."\"
                                           $sLogDateCondition");
         if (!DB::isError($DbResult))
         {
             // Event logs about meeting rooms registrations deleted
             $bLogsDeleted = TRUE;
         }

         return (($bRegistrationsAnonymized) && ($bLogsDeleted));
     }
