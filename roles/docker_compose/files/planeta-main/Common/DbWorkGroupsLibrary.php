...
/**
 * Treat workgroup registrations data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-06
 ...
 */
 function dbWorkGroupRegistrationsGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         $bLogsDeleted = FALSE;

         // We delete logs in relation with workgroup registrations
         $sLogDateCondition = " AND LogEventDate < ".date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("WorkGroupRegistrationDate", $ArrayParams)) && (count($ArrayParams["WorkGroupRegistrationDate"]) == 2))
         {
             $sLogDateCondition = " AND LogEventDate ".$ArrayParams["WorkGroupRegistrationDate"][0]
                                  ." \"".$ArrayParams["WorkGroupRegistrationDate"][1]."\"";
         }

         $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_WORKGROUP."\"
                                          AND LogEventService = \"".EVT_SERV_WORKGROUP_REGISTRATION."\" $sLogDateCondition");
         if (!DB::isError($DbResult))
         {
             // Event logs about workgroup registrations deleted
             $bLogsDeleted = TRUE;
         }
