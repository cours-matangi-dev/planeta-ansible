 ...
 *     - 2021-04-15 : v3.0. Taken into account forum topics, forum messages and forum subscribtions,
 *                          taken into account OBJ_FORUM_MESSAGE in uploaded files
 *     - 2022-06-07 : v3.1. Taken into account EVT_BILL and EVT_SERV_BILL
 *
 * @since 2012-01-12
 *
             ...
             if (in_array($Level, $GLOBALS['CONF_LOG_EVENTS_LEVELS']))
             {
                 ...
                 switch($ItemType)
                 {
                                 ...
                                 if (isset($Record))
                                 {
                                     ...
                                     switch($Record['UploadedFileObjectType'])
                                     {
                                         ...
                                         case OBJ_FORUM_MESSAGE:
                                             $bObjectExists = isExistingForumMessage($DbConnection, $Record['ObjectID']);
                                             $RecordLinkedObject = getTableRecordInfos($DbConnection, 'ForumMessages', $Record['ObjectID']);
                                             $Name = getTableFieldValue($DbConnection, 'ForumTopics', $RecordLinkedObject['ForumTopicID'],
                                                                        'ForumTopicTitle');
                                             $Type = $GLOBALS['LANG_FORUM_MESSAGE'];
                                             break;
                                     }
                         ...
                         switch($Service)
                         {
                             case EVT_SERV_BILL:
                                 ...
                                 switch($Action)
                                 {
                                     case EVT_ACT_ADD:
                                     case EVT_ACT_UPDATE:
                                         if (isExistingBill($DbConnection, $ItemID))
                                         {
                                             $Record = getTableRecordInfos($DbConnection, 'Bills', $ItemID);
                                             $LinkedObjectID = $ItemID;
                                         }
                                         break;
                                 }
                                 ...
                                 if ((isset($Record)) && (isExistingFamily($DbConnection, $Record['FamilyID'])))
                                 {
                                     ...
                                     $Title = $GLOBALS['LANG_BILL']." $Reference";
                                 }
                                 break;
                         }
                         break;
                     ...
                     case EVT_FORUM:
                         switch($Service)
                         {
                             case EVT_SERV_FORUM_TOPIC:
                                 $ID = $ItemID;
                                 switch($Action)
                                 {
                                     case EVT_ACT_CREATE:
                                     case EVT_ACT_UPDATE:
                                         if (isExistingForumTopic($DbConnection, $ItemID))
                                         {
                                             $Record = getTableRecordInfos($DbConnection, 'ForumTopics', $ItemID);
                                         }
                                         break;

                                     case EVT_ACT_DELETE:
                                         if ((isset($ArrayInfos['ForumTopicDetails'])) && (!empty($ArrayInfos['ForumTopicDetails'])))
                                         {
                                             $Record = $ArrayInfos['ForumTopicDetails'];
                                             $LinkedObjectID = $Record['ForumCategoryID'];
                                         }
                                         break;
                                 }

                                 $Reference = $ItemID;
                                 $Name = $Record['ForumTopicTitle'];

                                 $Title = $GLOBALS['LANG_FORUM_TOPIC']." n∞$Reference : $Name";
                                 break;

                             case EVT_SERV_FORUM_MESSAGE:
                                 $ID = $ItemID;
                                 switch($Action)
                                 {
                                     case EVT_ACT_CREATE:
                                     case EVT_ACT_UPDATE:
                                         if (isExistingForumMessage($DbConnection, $ItemID))
                                         {
                                             $Record = getTableRecordInfos($DbConnection, 'ForumMessages', $ItemID);
                                         }
                                         break;

                                     case EVT_ACT_DELETE:
                                         if ((isset($ArrayInfos['ForumMessageDetails'])) && (!empty($ArrayInfos['ForumMessageDetails'])))
                                         {
                                             $Record = $ArrayInfos['ForumMessageDetails'];
                                             $LinkedObjectID = $Record['ForumTopicID'];
                                         }
                                         break;
                                 }

                                  // Get infos about the concerned forum topic
                                 $RecordTopic = getTableRecordInfos($DbConnection, 'ForumTopics', $Record['ForumTopicID']);

                                 $RecordSupporter = getSupportMemberInfos($DbConnection, $Record['SupportMemberID']);
                                 ...
                                 $Title = $GLOBALS['LANG_FORUM_MESSAGE'];
                                 $Subject = $RecordTopic['ForumTopicTitle'];
                                 break;

                             case EVT_SERV_FORUM_TOPIC_SUBSCRIBTION:
                                 $ID = $ItemID;
                                 switch($Action)
                                 {
                                     case EVT_ACT_ADD:
                                         if (isExistingForumTopicSubscribtion($DbConnection, $ItemID))
                                         {
                                             $Record = getTableRecordInfos($DbConnection, 'ForumTopicsSubscribtions', $ItemID);
                                         }
                                         break;

                                     case EVT_ACT_DELETE:
                                         if ((isset($ArrayInfos['ForumTopicSubscribtionDetails'])) && (!empty($ArrayInfos['ForumTopicSubscribtionDetails'])))
                                         {
                                             $Record = $ArrayInfos['ForumTopicSubscribtionDetails'];
                                             $LinkedObjectID = $Record['ForumTopicID'];
                                         }
                                         break;
                                 }
                                 ...
                                 $Supporter .= $RecordSupporter['SupportMemberLastname'];
                                 $Title = $GLOBALS['LANG_FORUM_TOPIC_SUBSCRIBTION'];
                                 $Subject = $RecordTopic['ForumTopicTitle'];
                                 break;
 ...
 *
 * @author Christpohe Javouhey
 * @version 1.0
 * @since 2021-05-10
 *
 * @param $Value       Integer      Value to convert in binary
 *
 * @return Array of Integers        Coeff of binary value ([0] less significant bit, [n-1] most significant bit),
 *                                  empty array otherwise
 */
 function convInt2Bin($Value)
 {
     $ArrayCoeffBin = array();
 ...
 * @version 2.0
 *     -2021-09-10 : replace create_function() by anonymous function for PHP8
 *
 * @since 2007-10-05
 *
 ...
 * @author Christophe Javouhey
 * @version 1.1
 *     - 2021-01-04 : v1.1. Remove {} and use substr() to extract 1 character from a string (for PHP 7.4)
 *
 * @since 2019-08-30
 *
 ...
 *     - 2021-09-10 : replace eregi by mb_eregi for PHP8
 *
 * @since 2005-01-27
 *
 *
 ...
 */
 function getBrowserName()
 {
     ...
     if (mb_eregi('msie', $_SERVER["HTTP_USER_AGENT"]) && !mb_eregi('opera', $_SERVER["HTTP_USER_AGENT"]))
     {
         // Internet Explorer
         return "IE";
     }
     elseif (mb_eregi('opera', $_SERVER["HTTP_USER_AGENT"]))
     {
         // Opera
         return "OPERA";
     }
     elseif (mb_eregi('Mozilla/4.', $_SERVER["HTTP_USER_AGENT"]))
     {
         // Netscape 4.x
         return "NS4-$sOSname";
     }
     elseif (mb_eregi('Mozilla/5.0', $_SERVER["HTTP_USER_AGENT"]) && !mb_eregi('Konqueror', $_SERVER["HTTP_USER_AGENT"]))
     {
         // Netscape 6
         return "NS6-$sOSname";
     }
 ...
 */
 ...
 {
     $sBrowserVersion = '';
     if (mb_eregi('Mozilla/5.0', $_SERVER["HTTP_USER_AGENT"]) && !mb_eregi('Konqueror', $_SERVER["HTTP_USER_AGENT"]))
     {
