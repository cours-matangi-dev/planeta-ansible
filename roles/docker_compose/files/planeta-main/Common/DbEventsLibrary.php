...
/**
 * Treat events data of a support member for GDPR, thanks to its ID
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-07
 ...
 *                                      FALSE otherwise
 */
 function dbEventsGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     // The parameters are correct?
     if ($AnonymizedSupportMemberID > 0)
     {
         $ArrayConcernedID = array();
         $bEventsAnonymized = FALSE;
         $bLogsDeleted = FALSE;

         // First, we select concerned events
         $sDateCondition = " AND e.EventStartDate < ".date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("EventStartDate", $ArrayParams)) && (count($ArrayParams["EventStartDate"]) == 2))
         {
             $sDateCondition = " AND e.EventStartDate ".$ArrayParams["EventStartDate"][0]
                              ." \"".$ArrayParams["EventStartDate"][1]."\"";
         }
         ...
         if (!DB::isError($DbResult))
         {
             if ($DbResult->numRows() >= 0)
             {
                 while($Record = $DbResult->fetchRow(DB_FETCHMODE_ASSOC))
                 {
                     $ArrayConcernedID[] = $Record['EventID'];
                 }
             }
         }
         ...
         {
             $bEventsAnonymized = TRUE;
             $bLogsDeleted = TRUE;
         }
         else
         {
             // Now, we anonymized concerned events
             $DbResult = $DbConnection->query("UPDATE Events SET SupportMemberID = $AnonymizedSupportMemberID
                                               WHERE EventID IN ".constructSQLINString($ArrayConcernedID));
             if (!DB::isError($DbResult))
             {
                 // Events anonymized
                 $bEventsAnonymized = TRUE;
             ...
             $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_EVENT."\"
                                               AND LogEventService = \"".EVT_SERV_EVENT."\"
                                               AND LogEventItemID IN ".constructSQLINString($ArrayConcernedID));
             if (!DB::isError($DbResult))
             {
                 // Event logs about events deleted
                 $bLogsDeleted = TRUE;
             }
         }

         return (($bEventsAnonymized) && ($bLogsDeleted));
     }
...
/**
 * Treat event registrations data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-07
 ...
 */
 function dbEventRegistrationsGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         $ArrayConcernedID = array();
         ...
         // First, we store in the Stats table the too old event registrations
         $sDateCondition = " AND e.EventStartDate < ".date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("EventStartDate", $ArrayParams)) && (count($ArrayParams["EventStartDate"]) == 2))
         {
             $sDateCondition = " AND e.EventStartDate ".$ArrayParams["EventStartDate"][0]." \"".$ArrayParams["EventStartDate"][1]."\"";
         }
         ...
         if ((array_key_exists("SupportMemberID", $ArrayParams)) && (!empty($ArrayParams["SupportMemberID"])))
         {
             ...
             if (is_array($ArrayParams["SupportMemberID"]))
             {
                 $sSupporterCondition = " AND er.SupportMemberID IN ".constructSQLINString($ArrayParams["SupportMemberID"]);
             }
             else
             {
                 $sSupporterCondition = " AND er.SupportMemberID = ".$ArrayParams["SupportMemberID"];
             }
         }

         $DbResult = $DbConnection->query("SELECT e.EventStartDate AS StatPeriod,
                                           '".STAT_TYPE_NB_EVENT_REGISTRATIONS."' AS StatType,
                                           e.EventID AS StatSubType, COUNT(er.EventRegistrationID) AS StatValue
                                           FROM Events e, EventRegistrations er
                                           WHERE e.EventID = er.EventID $sDateCondition $sSupporterCondition
         ...
         // Then, we get ID of concerned event registrations
         $DbResult = $DbConnection->query("SELECT er.EventRegistrationID
                                           FROM Events e, EventRegistrations er
                                           WHERE e.EventID = er.EventID $sDateCondition $sSupporterCondition");
         if (!DB::isError($DbResult))
         {
             if ($DbResult->numRows() >= 0)
             {
                 while($Record = $DbResult->fetchRow(DB_FETCHMODE_ASSOC))
                 {
                     $ArrayConcernedID[] = $Record['EventRegistrationID'];
                 }
             }
         }
         ...
         {
             // Now, we delete the selected event registrations too old
             $DbResult = $DbConnection->query("DELETE FROM EventRegistrations
                                               WHERE EventRegistrationID IN ".constructSQLINString($ArrayConcernedID));
             if (!DB::isError($DbResult))
             {
             ...
             // Then, we delete event logs
             $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_EVENT."\"
                                               AND LogEventService = \"".EVT_SERV_EVENT_REGISTRATION."\"
                                               AND LogEventItemID IN ".constructSQLINString($ArrayConcernedID));
             if (!DB::isError($DbResult))
             {
                 // Event logs about event registrations deleted
                 $bLogsDeleted = TRUE;
             }
         }
...
/**
 * Treat event swapped registrations data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-10
 ...
 * @return Boolean                      TRUE if GDPR treatment is done, FALSE otherwise
 */
 function dbEventSwappedRegistrationsGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         $ArrayConcernedID = array();
         ...
         // First, we get ID of concerned event swapped registrations
         $sDateCondition = " AND e.EventStartDate < ".date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("EventStartDate", $ArrayParams)) && (count($ArrayParams["EventStartDate"]) == 2))
         {
             $sDateCondition = " AND e.EventStartDate ".$ArrayParams["EventStartDate"][0]." \"".$ArrayParams["EventStartDate"][1]."\"";
         ...
         if ((array_key_exists("SupportMemberID", $ArrayParams)) && (!empty($ArrayParams["SupportMemberID"])))
         {
             ...
             if (is_array($ArrayParams["SupportMemberID"]))
             {
                 $sSupporterCondition = " AND esr.SupportMemberID IN ".constructSQLINString($ArrayParams["SupportMemberID"]);
             }
             else
             {
                 $sSupporterCondition = " AND esr.SupportMemberID = ".$ArrayParams["SupportMemberID"];
             }
         }

         $DbResult = $DbConnection->query("SELECT esr.EventSwappedRegistrationID
         ...
         if (!DB::isError($DbResult))
         {
             if ($DbResult->numRows() >= 0)
             {
                 while($Record = $DbResult->fetchRow(DB_FETCHMODE_ASSOC))
                 {
                     $ArrayConcernedID[] = $Record['EventSwappedRegistrationID'];
                 }
             }
         }
         ...
         {
             // Now, we delete the selected event swapped registrations too old
             $DbResult = $DbConnection->query("DELETE FROM EventSwappedRegistrations
                                               WHERE EventSwappedRegistrationID IN ".constructSQLINString($ArrayConcernedID));
             if (!DB::isError($DbResult))
             {
                 // Event swapped registrations deleted
                 $bRegistrationsDeleted = TRUE;
             }

             // Then, we delete event logs
             $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_EVENT."\"
                                               AND LogEventService = \"".EVT_SERV_EVENT_SWAPPED_REGISTRATION."\"
                                               AND LogEventItemID IN ".constructSQLINString($ArrayConcernedID));
             if (!DB::isError($DbResult))
             {
                 // Event logs about event swapped registrations deleted
                 $bLogsDeleted = TRUE;
             }
         }

         return (($bRegistrationsDeleted) && ($bLogsDeleted));
     }
