 ...
 *
 * @return Integer                         Nursery registration ID if is exists for this child and date,
 *                                         0 otherwise
 */
 ...
 */
 function getChildrenListForNurseryPlanning($DbConnection, $StartDate, $EndDate = NULL, $OrderBy = 'ChildClass', $bActivated = FALSE, $Mode = PLANNING_BETWEEN_DATES, $ArrayParams = array())
 {
     if (empty($EndDate))
     {
...
/**
 * Treat nursery registrations data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-05
 ...
 */
 function dbNurseryRegistrationsGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         ...
         // First, we store in the Stats table the too old nursery registrations
         $sDateCondition = " NurseryRegistrationForDate < ".date('Y-m-d', strtotime("5 years ago"));
         $sLogDateCondition = " AND LogEventDate < ".date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("NurseryRegistrationForDate", $ArrayParams)) && (count($ArrayParams["NurseryRegistrationForDate"]) == 2))
         {
             $sDateCondition = " NurseryRegistrationForDate ".$ArrayParams["NurseryRegistrationForDate"][0]
                               ." \"".$ArrayParams["NurseryRegistrationForDate"][1]."\"";

             $sLogDateCondition = " AND LogEventDate ".$ArrayParams["NurseryRegistrationForDate"][0]
                                  ." \"".$ArrayParams["NurseryRegistrationForDate"][1]."\"";
         }

         // To treat other timeslots, we get the max number of timeslot
         $iMaxOtherTimeSlot = NULL;
         $sOtherTimeSlotSQL = '';
         $DbResult = $DbConnection->query("SELECT MAX(NurseryRegistrationOtherTimeslots) AS MaxOtherTimeSlot
                                           FROM NurseryRegistrations ");
         ...
         if (!DB::isError($DbResult))
         {
             ...
             {
                 $Record = $DbResult->fetchRow(DB_FETCHMODE_ASSOC);
                 if (!empty($Record['MaxOtherTimeSlot']))
                 {
                      $iMaxOtherTimeSlot = $Record['MaxOtherTimeSlot'];
                      $ArrayBin = convInt2Bin($iMaxOtherTimeSlot);
                      if (!empty($ArrayBin))
                      {
                          foreach($ArrayBin as $n => $Coeff)
                          {
                              if ($Coeff > 0)
                              {
                                  // We generate the SQL to do stat on this used timeslot
                                  $sOtherTimeSlotSQL .= " UNION SELECT DATE_FORMAT(NurseryRegistrationForDate, '%Y-%m') AS StatPeriod,
                                                         '".STAT_TYPE_NB_NURSERY_REGISTRATIONS."' AS StatType,
                                                         CONCAT(NurseryRegistrationChildGrade, '#SL$n') AS StatSubType,
                                                         COUNT(NurseryRegistrationID) AS StatValue
                                                         FROM NurseryRegistrations
                                                         WHERE $sDateCondition AND NurseryRegistrationOtherTimeslots & ".pow(2, $n)
         ...
         $DbResult = $DbConnection->query("SELECT DATE_FORMAT(NurseryRegistrationForDate, '%Y-%m') AS StatPeriod,
                                           '".STAT_TYPE_NB_NURSERY_REGISTRATIONS."' AS StatType,
                                           CONCAT(NurseryRegistrationChildGrade, '#AM') AS StatSubType,
                                           SUM(NurseryRegistrationForAM) AS StatValue
                                           FROM NurseryRegistrations
                                           WHERE $sDateCondition
                                           GROUP BY StatPeriod, StatType, StatSubType
                                           UNION
                                           SELECT DATE_FORMAT(NurseryRegistrationForDate, '%Y-%m') AS StatPeriod,
                                           '".STAT_TYPE_NB_NURSERY_REGISTRATIONS."' AS StatType,
                                           CONCAT(NurseryRegistrationChildGrade, '#PM') AS StatSubType,
                                           SUM(NurseryRegistrationForPM) AS StatValue
                                           FROM NurseryRegistrations
                                           WHERE $sDateCondition
                                           GROUP BY StatPeriod, StatType, StatSubType
                                           $sOtherTimeSlotSQL
         ...
         // Now, we delete the selected nursery registrations too old
         $DbResult = $DbConnection->query("DELETE FROM NurseryRegistrations WHERE $sDateCondition");
         if (!DB::isError($DbResult))
         {
             // Nursery registrations deleted
             $bRegistrationsDeleted = TRUE;
         }

         // Then, we delete event logs
         $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_NURSERY."\"
                                           AND LogEventService = \"".EVT_SERV_PLANNING."\" $sLogDateCondition");
         if (!DB::isError($DbResult))
         {
             // Event logs about nursery registrations deleted
             $bLogsDeleted = TRUE;
         }
