...
/**
 * Treat discounts of families data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-10
 ...
 */
 function dbDiscountsFamiliesGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         $ArrayConcernedID = array();
         $bDiscountsDeleted = FALSE;
         $bLogsDeleted = FALSE;

         // First, we get concerned discounts of families
         ...
         $DbResult = $DbConnection->query("SELECT df.DiscountFamilyID
                                           FROM DiscountsFamilies df, Families f
                                           WHERE df.FamilyID = f.FamilyID $sDateCondition
                                           ORDER BY df.DiscountFamilyID");
         ...
         if (!DB::isError($DbResult))
         {
             if ($DbResult->numRows() >= 0)
             {
                 while($Record = $DbResult->fetchRow(DB_FETCHMODE_ASSOC))
                 {
                     $ArrayConcernedID[] = $Record['DiscountFamilyID'];
                 }
             }
         }
         ...
         {
             $bDiscountsDeleted = TRUE;
             $bLogsDeleted = TRUE;
         }
         else
         {
             // We delete selected discounts
             $DbResult = $DbConnection->query("DELETE FROM DiscountsFamilies
                                               WHERE DiscountFamilyID IN ".constructSQLINString($ArrayConcernedID));
             if (!DB::isError($DbResult))
             {
                 // Discounts deleted
                 $bDiscountsDeleted = TRUE;
             ...
             $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_PAYMENT."\"
                                               AND LogEventService = \"".EVT_SERV_DISCOUNT."\"
                                               AND LogEventItemID IN ".constructSQLINString($ArrayConcernedID));
             if (!DB::isError($DbResult))
             {
                 // Event logs about discounts deleted
                 $bLogsDeleted = TRUE;
             }
         }

         return (($bDiscountsDeleted) && ($bLogsDeleted));
     }
