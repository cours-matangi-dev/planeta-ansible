...
/**
 * Treat exit permissions data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-06
 ...
 */
 function dbExitPermissionsGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         ...
         // First, we delete the selected exit permissions too old
         $sDateCondition = " ExitPermissionDate < ".date('Y-m-d', strtotime("5 years ago"));
         $sLogDateCondition = " AND LogEventDate < ".date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("ExitPermissionDate", $ArrayParams)) && (count($ArrayParams["ExitPermissionDate"]) == 2))
         {
             $sDateCondition = " ExitPermissionDate ".$ArrayParams["ExitPermissionDate"][0]
                               ." \"".$ArrayParams["ExitPermissionDate"][1]."\"";

             $sLogDateCondition = " AND LogEventDate ".$ArrayParams["ExitPermissionDate"][0]
                                  ." \"".$ArrayParams["ExitPermissionDate"][1]."\"";
         }

         $DbResult = $DbConnection->query("DELETE FROM ExitPermissions WHERE $sDateCondition");
         if (!DB::isError($DbResult))
         {
             // Exit permissions deleted
             $bRegistrationsDeleted = TRUE;
         }

         // Then, we delete event logs
         $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_EXIT_PERMISSION."\"
                                           $sLogDateCondition");
         if (!DB::isError($DbResult))
         {
             // Event logs about exit permeissions deleted
             $bLogsDeleted = TRUE;
         }
