...
/**
 * Treat snack registrations data for GDPR
 *
 * @author Christophe Javouhey
 * @version 1.0
 * @since 2021-05-06
 ...
 */
 function dbSnackRegistrationsGDPRTreatment($DbConnection, $AnonymizedSupportMemberID, $ArrayParams = array())
 {
     if ($AnonymizedSupportMemberID > 0)
     {
         ...
         // First, we delete the selected snack registrations too old
         $sDateCondition = " SnackRegistrationDate < ".date('Y-m-d', strtotime("5 years ago"));
         $sLogDateCondition = " AND LogEventDate < ".date('Y-m-d', strtotime("5 years ago"));
         if ((array_key_exists("SnackRegistrationDate", $ArrayParams)) && (count($ArrayParams["SnackRegistrationDate"]) == 2))
         {
             $sDateCondition = " SnackRegistrationDate ".$ArrayParams["SnackRegistrationDate"][0]
                               ." \"".$ArrayParams["SnackRegistrationDate"][1]."\"";

             $sLogDateCondition = " AND LogEventDate ".$ArrayParams["SnackRegistrationDate"][0]
                                  ." \"".$ArrayParams["SnackRegistrationDate"][1]."\"";
         }

         $DbResult = $DbConnection->query("DELETE FROM SnackRegistrations WHERE $sDateCondition");
         if (!DB::isError($DbResult))
         {
             // Snack registrations deleted
             $bRegistrationsDeleted = TRUE;
         }

         // Then, we delete event logs
         $DbResult = $DbConnection->query("DELETE FROM LogEvents WHERE LogEventItemType = \"".EVT_SNACK."\"
                                           $sLogDateCondition");
         if (!DB::isError($DbResult))
         {
             // Event logs about snack registrations deleted
             $bLogsDeleted = TRUE;
         }
