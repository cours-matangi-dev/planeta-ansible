...
/**
 * PHP check approved documents plugin module : display not approved document for the logged user
 *
 * @author Christophe Javouhey
 * @version 1.2
 *     - 2021-04-20 : v1.1. Only some support member state ID are concerned and if resultis empty
 *                    generate an empty <document> tag
 *     - 2021-11-09 : v1.2. Taken into account the creation date of the family to select documents to approve
 *
 * @since 2019-06-19
 */
 ...
 if (isSet($_SESSION["SupportMemberID"]))
 {
     ...
     if ((array_key_exists('getDocuments', $_GET)) && (strip_tags(trim($_GET['getDocuments'])) == 1))
     {
         ...
         {
             // Get creation date of the family
             $FamilyRecord = getTableRecordInfos($DbCon, 'Families', $_SESSION['FamilyID']);
             $sHaving = " AND DATE_FORMAT(da.DocumentApprovalDate, '%Y-%m-%d') >= '".date('Y-m-d', strtotime($FamilyRecord['FamilyDate']." 6 month ago"))."'";
         ...
         if (!empty($sCondition))
         {
             $DbResult = $DbCon->query("SELECT da.DocumentApprovalID, da.DocumentApprovalName, da.DocumentApprovalType, da.DocumentApprovalDate, dfa.DocumentFamilyApprovalID
                                        ...
                                        LEFT JOIN SupportMembers sm ON (dfa.SupportMemberID = sm.SupportMemberID AND sm.SupportMemberStateID IN (5))
                                        HAVING dfa.DocumentFamilyApprovalID IS NULL $sHaving
